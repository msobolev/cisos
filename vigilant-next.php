...
if($email !=''){
	...
	for($dm = 0; $dm < sizeof($banned_email_array); $dm++){
		if(strtoupper($domain_name)==strtoupper($banned_email_array[$dm])){
			if($action=='VigilantAppoints'){
				$url = "vigilant-appoints.php?contact_id=".$contact_id;
...
?>
	<html>
		...
		<input type="hidden" name="redirect" value="http://www.ctosonthemove.com/choose-signup-subscription.php?action=NextStep&nemail=<?=$email;?>&nfname=<?=$full_name;?>&ncid=<?=$contact_id?>&isp=<?=$isPresent?>" id="redirect_b8044a7d0a8cfea05d776e4548d6d2b8" />		
		<!--<input type="hidden" name="redirect" value="http://localhost:90/ananga/cto/choose-signup-subscription.php?action=NextStep&nemail=<?//=$email;?>&nfname=<?//=$full_name;?>&ncid=<?//=$contact_id?>" id="redirect_b8044a7d0a8cfea05d776e4548d6d2b8" />--> 
