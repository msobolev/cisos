...
$sql_query = "select * from " . TABLE_COTACT_US . " order by add_date desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'contact-us.php';
...
$cID = (isset($_GET['cID']) ? $_GET['cID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_COTACT_US . " where contact_id = '" . $cID . "'");
		 	com_redirect("contact-us.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("Partner deleted successfully"));
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($contact_id) ; $i++){
				com_db_query("delete from " . TABLE_COTACT_US . " where contact_id = '" . $contact_id[$i] . "'");
			}
		 	com_redirect("contact-us.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("Partner deleted successfully"));
	  ...
	  case 'detailes':
			...
			$query_edit=com_db_query("select * from " . TABLE_COTACT_US . "  where contact_id = '" . $cID . "'");
...
function confirm_del(nid,p){
	...
	if (agree)
		window.location = "contact-us.php?selected_menu=user&cID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "contact-us.php?selected_menu=user&cID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "contact-us.php?selected_menu=user";
	}	
...
<?php if(($action == '') || ($action == 'save')){	?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          ...
          <tr>
			...
			<form name="topicform" action="contact-us.php?action=alldelete&selected_menu=user" method="post">
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="contact-us.php?action=detailes&selected_menu=user&cID=<?=$data_sql['contact_id'];?>"><?=com_db_output($data_sql['name'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="45" border="0" align="center" cellpadding="0" cellspacing="0">
					  <tr> 
						<? if($btnDelete=='Yes'){ ?>
						<td align="center" valign="middle"><a href="#"><img src="images/small-delet-icon.gif" width="16" height="16" alt="Delete" countries="Delete" border="0" onclick="confirm_del('<?=$data_sql['contact_id'];?>','<?=$p;?>')" /></a><br />
						  Delete</td>
                        <? } ?>  
					  </tr>
					</table>				</td>
...
?>		
  	  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		 ...
		 <!--start iner table  -->
			...
			<tr>
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='contact-us.php?p=<?=$p;?>&cID=<?=$cID;?>&selected_menu=user'" /></td>
			</tr>
