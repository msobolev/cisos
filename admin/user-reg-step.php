...
$sql_query = "select * from " . TABLE_USER . " where subscription_id='0' and level='' order by res_date desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'user-reg-step.php';
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_USER . " where user_id = '" . $uID . "'");
		 	com_redirect("user-reg-step.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("User deleted successfully"));
	  ...
	  case 'detailes':
			...
			$name = com_db_output($data_edit['first_name']).' '.com_db_output($data_edit['last_name']);
...
function confirm_del(nid,p){
	...
	if (agree)
		window.location = "user-reg-step.php?selected_menu=user&uID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "user-reg-step.php?selected_menu=user&uID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "user-reg-step.php?selected_menu=user";
	}	
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="30%" align="left" valign="middle" class="heading-text">User Registration Only Step 1 Manager</td>
                  <td width="34%" align="left" valign="middle" class="message"><?=$msg?></td>
                  <?  if($btnDelete=='Yes'){  ?>
                  <td width="4%" align="right" valign="middle"><a href="#"><img src="images/delete-icon.jpg" border="0" width="25" height="28"  alt="Delete User" countries="Delete User" onclick="AllDelete('<?=$numRows?>');"  /></a></td>
          ...
          <tr>
			...
			<form name="topicform" action="user-reg-step.php?action=alldelete&selected_menu=user" method="post">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
			  <tr>
				...
				<td width="250" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">User Name</span></td>
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="user-reg-step.php?action=detailes&selected_menu=user&uID=<?=$data_sql['user_id'];?>"><?=com_db_output($data_sql['first_name']).' '.com_db_output($data_sql['last_name'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="45" border="0" align="center" cellpadding="0" cellspacing="0">
					  <tr> 
						<? if($btnDelete=='Yes'){  ?>
						<td align="center" valign="middle"><a href="#"><img src="images/small-delet-icon.gif" width="16" height="16" alt="Delete" countries="Delete" border="0" onclick="confirm_del('<?=$data_sql['user_id'];?>','<?=$p;?>')" /></a><br />
						  Delete</td>
                         <? } ?> 
					  </tr>
					</table>				</td>
...
?>		
  	  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">User Registration Only Step 1 Manager ::  Details </td>
		 ...
		 <!--start iner table  -->
			...
			<tr>
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='user-reg-step.php?p=<?=$p;?>&uID=<?=$uID;?>&selected_menu=user'" /></td>
			</tr>
