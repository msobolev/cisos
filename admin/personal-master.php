    ...
    switch ($action) {
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($personal_id) ; $i++){
				...
				com_db_query("delete from " . TABLE_MOVEMENT_MASTER . " where personal_id = '" . $personal_id[$i] . "'");
	  ...
	  case 'edit':
			...
			$edu_grad_year = com_db_output($data_edit['edu_grad_year']);
                        ...
                        $hr_sel = '';
                        $query_hr_edit = com_db_query("select * from " . TABLE_TOP_APPOINTMENTS . " where personal_id = '" . $pID . "' and top_appointments = 1");
                        $hr_count = com_db_num_rows($query_hr_edit);
                        if($hr_count > 0)
                            $hr_sel = 'checked';


                        $rec_sel = '';
                        $query_rec_edit = com_db_query("select * from " . TABLE_TOP_APPOINTMENTS . " where personal_id = '" . $pID . "' and top_appointments = 2");
                        $rec_count = com_db_num_rows($query_rec_edit);
                        if($rec_count > 0)
                            $rec_sel = 'checked';


                        $ben_sel = '';
                        //echo "<br>select * from " . TABLE_TOP_APPOINTMENTS . " where personal_id = '" . $pID . "' and top_appointments = 3";
                        //echo "<br>ben_sel: ".$ben_sel;
                        $query_ben_edit = com_db_query("select * from " . TABLE_TOP_APPOINTMENTS . " where personal_id = '" . $pID . "' and top_appointments = 3");

                        $ben_count = com_db_num_rows($query_ben_edit);
                        //echo "<br>ben count: ".$ben_count;   
                        if($ben_count > 0)
                        {
                            $data_ben_edit = com_db_fetch_array($query_ben_edit);
                            if(count($data_ben_edit) > 0)
                                $ben_sel = 'checked';
                        }
                        ...
                        $lea_sel = '';
                        $query_lea_edit = com_db_query("select * from " . TABLE_TOP_APPOINTMENTS . " where personal_id = '" . $pID . "' and top_appointments = 4");
                        $lea_count = com_db_num_rows($query_lea_edit);
                        if($lea_count > 0)
                        {    
                            $lea_sel = 'checked';        
                        }
		...
		case 'editsave':
			...
			$insert_id = $pID;
                        ...
                        if(isset($_POST['senior_hr']) && $_POST['senior_hr'] == 1)
                        {
                            $ins_hr_query = "INSERT into ".TABLE_TOP_APPOINTMENTS."(personal_id,top_appointments) values('$pID','1')";
                            com_db_query($ins_hr_query);
                        }
                        else
                        {
                            $del_hr_query = "DELETE from ".TABLE_TOP_APPOINTMENTS." where personal_id = '$pID' and top_appointments = 1;";
                            com_db_query($del_hr_query);
                        }    

                        if(isset($_POST['recruit_exe']) && $_POST['recruit_exe'] == 1)
                        {
                            $ins_rec_query = "INSERT into ".TABLE_TOP_APPOINTMENTS."(personal_id,top_appointments) values('$pID','2')";
                            com_db_query($ins_rec_query);
                        }
                        else
                        {
                            $del_rec_query = "DELETE from ".TABLE_TOP_APPOINTMENTS." where personal_id = '$pID' and top_appointments = 2;";
                            com_db_query($del_rec_query);
                        }

                        if(isset($_POST['benefit_exe']) && $_POST['benefit_exe'] == 1)
                        {
                            $ins_ben_query = "INSERT into ".TABLE_TOP_APPOINTMENTS."(personal_id,top_appointments) values('$pID','3')";
                            com_db_query($ins_ben_query);
                        }
                        else
                        {
                            $del_ben_query = "DELETE from ".TABLE_TOP_APPOINTMENTS." where personal_id = '$pID' and top_appointments = 3;";
                            com_db_query($del_ben_query);
                        }
                        

                        if(isset($_POST['learn_exe']) && $_POST['learn_exe'] == 1)
                        {
                            $ins_lea_query = "INSERT into ".TABLE_TOP_APPOINTMENTS."(personal_id,top_appointments) values('$pID','4')";
                            com_db_query($ins_lea_query);
                        }
                        else
                        {
                            $del_lea_query = "DELETE from ".TABLE_TOP_APPOINTMENTS." where personal_id = '$pID' and top_appointments = 4;";
                            com_db_query($del_lea_query);
                        }
	...
	if(lname==''){
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		 ...
		 <form name="frmDataEdit" id="frmDataEdit" method="post" enctype="multipart/form-data" action="personal-master.php?action=editsave&selected_menu=personal&pID=<?=$pID;?>&p=<?=$p;?>" onsubmit="return chk_form();">
            ...
            <tr>
                <td><input type="checkbox" name="senior_hr" id="senior_hr" value="1" <?=$hr_sel?> /> Top Appointments - Senior HR Executives</td>
            </tr>    
                      
            <tr>
                <td><input type="checkbox" name="recruit_exe" id="recruit_exe" value="1" <?=$rec_sel?> /> Top Appointments - Recruiting Executives</td>
            </tr>    
                      
            <tr>
                <td><input type="checkbox" name="benefit_exe" id="benefit_exe" value="1" <?=$ben_sel?> /> Top Appointments - Benefits Executives</td>
            </tr>    
                      
            <tr>
                <td><input type="checkbox" name="learn_exe" id="learn_exe" value="1" <?=$lea_sel?> /> Top Appointments - Learning and Development Executives</td>
            </tr>               
