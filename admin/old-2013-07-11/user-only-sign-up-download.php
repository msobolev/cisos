...
if($action == 'UserSignupDataExport'){
	...
	$xls->Text(4,3,"Sign Up Date");
		...
		while($download_row=com_db_fetch_array($result)) {
			...
			$addDt = $download_row['add_date'];
			if($addDt !=''){
				$addDt = explode('-',$addDt);
				$add_date = $addDt[1].'/'.$addDt[2].'/'.$addDt[0];
			}else{
				$add_date='';
			}
			...
			$xls->Text($xlsRow,3,"$add_date");
						
			$xlsRow++;
			}
		$xls->Output('user-signup-data-'. date('m-d-Y') . '.xls');	
		com_redirect("user-only-sign-up-download.php?selected_menu=user&msg=" . msg_encode("User signup data download successfully"));*/
