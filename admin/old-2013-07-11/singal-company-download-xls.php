...
if($company_id != ''){
	...
	$xls->Text(4,3,"Company Logo");
	$xls->Text(4,4,"Company Size Revenue");
	$xls->Text(4,5,"Company Size Employees");
	$xls->Text(4,6,"Industry");
	$xls->Text(4,7,"Email Pattern");
	$xls->Text(4,8,"Leadership Page");
	$xls->Text(4,9,"Address");
	$xls->Text(4,10,"Address 2");
	$xls->Text(4,11,"City");
	$xls->Text(4,12,"Country");
	$xls->Text(4,13,"State");
	$xls->Text(4,14,"Zip Code");
	$xls->Text(4,15,"Phone");
	$xls->Text(4,16,"Fax");
	$xls->Text(4,17,"About Company");
	$xls->Text(4,18,"Facebook Link");
	$xls->Text(4,19,"Linkedin Link");
	$xls->Text(4,20,"Twitter Link");
	$xls->Text(4,21,"Google+ Link");		

	
	
	
	$download_query_string = "select c.company_name,c.company_website,c.company_logo,r.name as company_revenue,e.name as company_employee,
		...
		while($download_row=com_db_fetch_array($result)) {
			...
			$about_company = str_replace(',',';',com_db_output(strip_tags($download_row['about_company'])));
			...
			$xls->Text($xlsRow,3,"$company_logo");
			$xls->Text($xlsRow,4,"$company_revenue");
			$xls->Text($xlsRow,5,"$company_employee");
			$xls->Text($xlsRow,6,"$company_industry");
			$xls->Text($xlsRow,7,"$email_patternc");
			$xls->Text($xlsRow,8,"$leadership_page");
			$xls->Text($xlsRow,9,"$address");
			$xls->Text($xlsRow,10,"$address2");
			$xls->Text($xlsRow,11,"$city");
			$xls->Text($xlsRow,12,"$country");
			$xls->Text($xlsRow,13,"$state");
			$xls->Text($xlsRow,14,"$zip_code");
			$xls->Text($xlsRow,15,"$phone");
			$xls->Text($xlsRow,16,"$fax");
			$xls->Text($xlsRow,17,"$about_company");
			$xls->Text($xlsRow,18,"$facebook_link");
			$xls->Text($xlsRow,19,"$linkedin_link");
			$xls->Text($xlsRow,20,"$twitter_link");
			$xls->Text($xlsRow,21,"$googleplush_link");
