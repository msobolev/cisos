					...
					if($get_ext=='csv'){
						...
						if(move_uploaded_file($data_file , $destination_file)){
							...
							if($cap_flag==1){
								...
								com_redirect("contact-master-import.php?selected_menu=master&msg=" . msg_encode($msg)."&error_list=".msg_encode($error_list));
							}
							//caption end
							for($i=$columnheadings; $i<sizeof($filecontents); $i++) {
								...
								if(sizeof($newArray)== 14){
									$col_fld = array('company_name','company_website','company_revenue','company_employee','company_industry','ind_group_id','industry_id','email_pattern','leadership_page','address','address2','city','country','state','zip_code','about_company','add_date');
								
									$strQuery = "INSERT INTO " . TABLE_CONTACT_MASTER . " (";
									...
									$isCompanyWebsite = com_db_GetValue("select contact_id from ".TABLE_CONTACT_MASTER." where company_website='".$import_company_website."'");
										if($isCompanyWebsite>0){
											$modify_date = date('Y-m-d');
											$update_contact = "update ".TABLE_CONTACT_MASTER." set company_name='".$import_company_name."',company_revenue='".$revenue_size_id ."', company_employee='".$employee_size_id."',
															   ...
															   address2='".$address2."',city='".$import_city."',state='".$state_id."',country='".$country_id."',zip_code='".$import_zip_code."',about_company='".$import_about_company."',modify_date='".$modify_date."' where contact_id='".$isCompanyWebsite."'";
											com_db_query($update_contact);
										}else{
											com_db_query($strQuery2);
										}
									}else{
									 $msg = "File column numbers is ".sizeof($newArray).', But Import required is 14 columns';
									 com_redirect("contact-master-import.php?selected_menu=master&msg=" . msg_encode($msg));	
									}
							}
    ...
    <td align="left" valign="top" bgcolor="#FFFFFF"><table width="975" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <form name="importfile" method="post" action="contact-master-import.php?selected_menu=master&action=save" enctype="multipart/form-data" onsubmit="return checkForm();">	
