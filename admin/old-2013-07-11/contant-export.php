...
if($action == 'ContactExport'){
	...
	if($from_date !='' ){
		...
		if(strlen($from_date)==10 && is_numeric($fd[2]) && is_numeric($fd[1]) && is_numeric($fd[0])){
			...
			com_redirect("contant-export.php?selected_menu=contact&msg=" . msg_encode("Please enter valid date format ".$check));
		}	
	}
	if($to_date !=''){
		$td = explode('/',$to_date);
		if(strlen($to_date)==10 && is_numeric($td[2]) && is_numeric($td[1]) && is_numeric($td[0])){
			...
			com_redirect("contant-export.php?selected_menu=contact&msg=" . msg_encode("Please enter valid date format"));
		}	
	}
	...
	$xls->Text(4,30,"Not Current?");
	
	$download_query_string = "select c.first_name,c.middle_name,c.last_name,c.new_title,c.email,c.phone,c.contact_url,
							c.company_name,c.company_website,c.not_current,r.name as company_revenue,e.name as company_employee,
		...
		if($new_title==''){
			if($from_date!='' && $to_date !=''){
				$download_query = $download_query_string ."	where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and  c.status='0' and announce_date >='".$from_date."' and announce_date <='".$to_date."'";
			}else{
				$download_query = $download_query_string ."	where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and  c.status='0'";
			}
			 ...
			 if($from_date!='' && $to_date !=''){	
			 	$download_query = $download_query_string ." where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and c.new_title not in (" . $title_list . ") and announce_date >='".$from_date."' and announce_date <='".$to_date."'";
			 }else{
			 	$download_query = $download_query_string ." where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and c.new_title not in (" . $title_list . ")";
			 }
		}else{
			 if($from_date!='' && $to_date !=''){	
			 	$download_query = $download_query_string ." where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and c.new_title = '" . $new_title . "' and announce_date >='".$from_date."' and announce_date <='".$to_date."'";
			 }else{
			 	$download_query = $download_query_string ." where c.company_revenue=r.id and c.company_employee=e.id and c.company_industry=i.industry_id and c.state=s.state_id and c.country=ct.countries_id and c.source=so.id and c.movement_type=m.id and c.new_title = '" . $new_title . "'";
			 }
		}
		//remove
		if(is_array($state) && $state !=''){
			...
			if($state_list !=''){
				$download_query = $download_query . " and c.state in (".$state_list.")";
			}
		}
		
		if($sortby !=''){
			$download_query = $download_query .' order by c.'.$sortby;
		}
		...
		while($download_row=com_db_fetch_array($result)) {
			...
			$xls->Text($xlsRow,30,"$not_current");
			
			$xlsRow++;
			}
		$xls->Output('download-contact-'. date('m-d-Y') . '.xls');	
		com_redirect("contact-export.php?selected_menu=contact&msg=" . msg_encode("Contacts download successfully"));
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			<div id="spiffycalendar" class="text"></div>
				...
				//--></script>	
		  <form name="exportfile" method="post" action="contant-export.php?action=ContactExport">
