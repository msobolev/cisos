...
if($type=='AddCompanyInfo'){
	...
	if($isPresentUrl !=''){
		$company_details_result = com_db_query("select company_name,company_revenue,company_employee,company_industry,ind_group_id,address,address,address2,city,state,country,zip_code,about_company from ".TABLE_CONTACT." where company_website='".$q."'");
		...
		$all_result = $company_name.'###'.$company_revenue.'###'.$company_employee.'###'.$company_industry1.'###'.$address.'###'.$address2.'###'.$city.'###'.$country1.'###'.$state.'###'.$zip_code.'###'.$about_company;
		echo $all_result;
	}
}
?> 