...
if($action == 'UserDataExport'){
	...
	$xls->Text(4,3,"Company Name");
	$xls->Text(4,4,"Phone");
	$xls->Text(4,5,"Email");
	$xls->Text(4,6,"Expire Date");
	$xls->Text(4,7,"Register Date");
	$xls->Text(4,8,"Sub Type");
	$xls->Text(4,9,"Total Login");
	$xls->Text(4,10,"Total Searches");
	$xls->Text(4,11,"Total Downloads");
		...
		while($download_row=com_db_fetch_array($result)) {
			...
			$xls->Text($xlsRow,3,"$company_name");
			$xls->Text($xlsRow,4,"$phone");
			$xls->Text($xlsRow,5,"$email");
			$xls->Text($xlsRow,6,"$exp_date");
			$xls->Text($xlsRow,7,"$res_date");
			$xls->Text($xlsRow,8,"$subscription_name");
			$xls->Text($xlsRow,9,"$total_login");
			$xls->Text($xlsRow,10,"$total_search");
			$xls->Text($xlsRow,11,"$total_download");
