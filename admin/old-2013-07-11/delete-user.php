...
if($action == 'UserSearchResult'){
	...
	if($search_qry==''){
		$sql_query = "select * from " . TABLE_DELETE_USER . " order by user_id desc";
	}else{
		$sql_query = "select * from " . TABLE_DELETE_USER . " where ". $search_qry." order by user_id desc";
	}
...
$uID = (isset($_GET['uID']) ? $_GET['uID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_DELETE_USER . " where user_id = '" . $uID . "'");
		 	com_redirect("delete-user.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("User deleted successfully"));
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($user_id) ; $i++){
				com_db_query("delete from " . TABLE_DELETE_USER . " where user_id = '" . $user_id[$i] . "'");
			}
		 	com_redirect("delete-user.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("User deleted successfully"));
	  ...
	  case 'edit':
	   		...
	   		$query_edit=com_db_query("select * from " . TABLE_DELETE_USER . " where user_id = '" . $uID . "'");
		...
		case 'editsave':
			...
			$Free_Paid = com_db_GetValue("select level from " .TABLE_DELETE_USER." where user_id='".$uID."'");
			
			$query = "update " . TABLE_DELETE_USER . " set first_name = '" . $first_name . "',  last_name = '" . $last_name . "', title = '" . $title . "', company_name = '".$company_name."', phone = '".$phone."', email='".$email."', password = '" . $password ."', modify_date = '".$modify_date."', exp_date='".$exp_date."',res_date='".$reg_date."',subscription_id='".$subscription_id."', status='".$sub_status."', level ='".$sub_level."'  where user_id = '" . $uID . "'";
			com_db_query($query);
			
			if($Free_Paid != $sub_level){
				$query = "update " . TABLE_DELETE_USER . " set payment_by = 'Admin' where user_id = '" . $uID . "'";
				com_db_query($query);
	  		...
	  		com_redirect("delete-user.php?p=". $p ."&uID=" . $uID . "&selected_menu=user&msg=" . msg_encode("User update successfully"));
	...
	case 'addsave':
			...
			$query = "insert into " . TABLE_DELETE_USER . " (first_name, last_name, title, company_name, phone, email, password, subscription_id, exp_date, res_date, accept, status) values ('$first_name', '$last_name', '$title','$company_name','$phone','$email','$password','1','$exp_date','$added','1','0')";
			com_db_query($query);
	  		com_redirect("delete-user.php?p=" . $p . "&selected_menu=user&msg=" . msg_encode("New news added successfully"));
	...
	case 'activate':
	   		...
	   		$status=$_GET['status'];
			if($status==0){
				$query = "update " . TABLE_DELETE_USER . " set status = '1' where user_id = '" . $uID . "'";
			}else{
				$query = "update " . TABLE_DELETE_USER . " set status = '0' where user_id = '" . $uID . "'";
			}	
			com_db_query($query);
	  		com_redirect("delete-user.php?p=". $p ."&uID=" . $uID . "&selected_menu=user&msg=" . msg_encode("User update successfully"));
...
function confirm_del(nid,p){
	...
	if (agree)
		window.location = "delete-user.php?selected_menu=user&uID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "delete-user.php?selected_menu=user&uID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "delete-user.php?selected_menu=user";
	}	
...
function confirm_artivate(nid,p,status){
	...
	if (agree)
		window.location = "delete-user.php?selected_menu=user&uID=" + nid + "&p=" + p + "&status=" + status + "&action=activate";
	else
		window.location = "delete-user.php?selected_menu=user&uID=" + nid + "&p=" + p ;
}

function UserSearch(){
	window.location ='delete-user.php?action=UserSearch&selected_menu=user';
}
...
function PaymentClose(pi){
 ...
 <tr>
	...
	<div id="light" class="white_content" style="display:<? if($action=='UserSearch'){ echo 'block';} else { echo 'none'; } ?>;">
		...
		//--></script>
		<form name="frmSearch" id="frmSearch" method="post" action="delete-user.php?selected_menu=user&action=UserSearchResult">
		<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
		  ...
		  <tr>
			<td align="center" valign="top">&nbsp;</td>
		  	<td align="left" valign="top"><input type="submit" value="Search" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='delete-user.php?selected_menu=user'" /></td>
		  </tr>
		  <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="25%" align="left" valign="middle" class="heading-text">Duplicate Email Manager</td>
                  <td width="63%" align="left" valign="middle" class="message"><?=$msg?></td>
                  ...
                  <td width="4%" align="right" valign="middle"><a href="#"><img src="images/add-icon.jpg" border="0" width="25" height="28" alt="Add User" title="Add User" onclick="window.location='delete-user.php?action=add&selected_menu=user'"  /></a></td>
          ...
          <tr>
			...
			<form name="topicform" action="delete-user.php?action=alldelete&selected_menu=user" method="post">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
			  <tr>
				<td width="26" height="30" align="center" valign="middle" class="right-border-left"><span class="right-box-title-text">#</span></td>
				...
				<td width="221" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Name</span> </td>
				<td width="128" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Subscription</span> </td>
                <td width="159" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">E-mail</span> </td>
                <td width="105" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Date</span> </td>
                <td width="92" height="30" align="center" valign="middle" class="left-border"><div align="center"><span class="right-box-title-text">Action</span></div></td>
			  </tr>
			<?php
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="delete-user.php?action=detailes&selected_menu=user&uID=<?=$data_sql['user_id'];?>"><?=com_db_output($data_sql['first_name']).' '.com_db_output($data_sql['last_name'])?></a></td>
				<td height="30" align="left" valign="middle" class="right-border-text"><?=$subscription_name;?></td>
                <td height="30" align="left" valign="middle" class="right-border-text"><?=$email;?></td>
					  ...
					  <tr> 
					  	<!--<?php// if($status==0){ ?>
					   	...
					   	<td width="24%" align="center" valign="middle"><a href="#"><img src="images/icon/inactive-icon.gif" width="16" height="16" alt="" title="" border="0" onclick="confirm_artivate('<?//=$data_sql['user_id'];?>','<?//=$p;?>','<?//=$status;?>');" /></a><br />
					   	  Status</td>
						<?php// } ?>
						<td width="23%" align="center" valign="middle"><a href="#"><img src="images/small-edit-icon.gif" width="16" height="16" alt="Edit" title="Edit" border="0" onclick="window.location='delete-user.php?selected_menu=user&p=<?//=$p;?>&uID=<?//=$data_sql['user_id'];?>&action=edit'" /></a><br />
						  Edit</td>-->
		 ...
		 <!--start iner table  -->
          ...
          <table width="95%" align="left" cellpadding="5" cellspacing="5" border="0">
			...
			<tr>
				...
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='delete-user.php?p=<?=$p;?>&uID=<?=$uID;?>&selected_menu=user'" /></td>
			</tr>
...
?>
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		 ...
		 <!--start iner table  -->
			 ...
			 <form name="frm_alert" id="frm_alert" method="post" action="delete-user.php?action=AlertConfirm&selected_menu=user&uID=<?=$uID?>">
				  ...
				  <table width="490" border="0" align="center" cellpadding="2" cellspacing="0">
					  <tr>
						...
						<td width="278" align="left" valign="middle"><input name="title" type="text" id="title" size="46" value="<? if($title==''){echo 'Type in the Title';}else{echo $title;}?>" class="list-field" onfocus=" if (this.value == 'Type in the Title') { this.value = ''; };" onblur="if (this.value == '') { this.value='Type in the Title';};"/>
							<!--<select name="title" id="title" style="width:300px;">
								<option value="">Any</option>
								<?//=selectComboBox("select title,title from ".TABLE_TITLE,$title)?>
							</select>-->	
					  ...
					  <td width="247"  align="left" valign="top">
						...
						<select name="industry" id="industry" style="width:300px;">
							...
							<!--<table width="280" border="0" align="left" cellpadding="0" cellspacing="0" class="my_div">
								<tr>
								  <td valign="top"><input name="industry" class="text-new-227" type="text" id="industry" style="margin-top:0px; border:0px solid #000000; color:#737373;" onfocus="fieldHighlightCombo('industry');MyCombo_Open('div_industry');AllComboDivCloseAlert('div_industry');" onkeyup="AddIndustry('industry');" size="34"  value="<? if($industry == ''){echo 'Any';}else{ echo $industry;}?>" onblur="fieldLosslightCombo('industry');"/>
								  <img src="images/down.png" align="right" width="20" height="20" onclick="MyCombo_Open('div_industry');"/> </td>
								</tr>
								<tr>
									<td valign="top">
										<div id="div_industry" class="industry" style="display:none; position:absolute;">
											<?//=DivContentIndustryLoad('div_industry','industry','title',TABLE_INDUSTRY)?>
										</div>
									</td>
								</tr>
							</table>-->					  </td>
					...
					<tr>
					  ...
					  <td width="247"  align="left" valign="middle">
							...
							<!--<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="my_div">
								<tr>
								  <td valign="top">
										<input type="text" name="revenue_size" id="revenue_size" class="text-new-227" style="margin-top:0px; border:0px solid #000000; color:#737373;" onfocus="fieldHighlightCombo('revenue_size');MyCombo_Open('div_revenue_size');AllComboDivCloseAlert('div_revenue_size');" size="34" value="<? if($revenue_size==''){echo 'Any';}else{ echo $revenue_size;}?>" onblur="fieldLosslightCombo('revenue_size');"/>
										<img src="images/down.png" align="right" width="20" height="20" onclick="MyCombo_Open('div_revenue_size');"/>								</td>
								</tr>
								<tr>
									<td valign="top">
										<div id="div_revenue_size" class="inner" style="display:none; position:absolute;">
											<?//=DivContentLoad('div_revenue_size','revenue_size','name',TABLE_REVENUE_SIZE)?>
										</div>
									</td>
								</tr>
							</table>-->					  </td>
					...
					<tr>
					  ...
					  <td width="247"  align="left" valign="top">
						...
						<!--<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="my_div">
							<tr>
							  <td valign="top">
									<input type="text" name="employee_size" id="employee_size" class="text-new-227" style="margin-top:0px; border:0px solid #000000; color:#737373;" onfocus="fieldHighlightCombo('employee_size');MyCombo_Open('div_employee_size');AllComboDivCloseAlert('div_employee_size');" size="34" value="<? if($employee_size==''){echo 'Any';}else{echo $employee_size;}?>" onblur="fieldLosslightCombo('employee_size');"/>
									<img src="images/down.png" align="right" width="20" height="20" onclick="MyCombo_Open('div_employee_size');"/>								</td>
							</tr>
							<tr>
								<td valign="top">
									<div id="div_employee_size" class="inner" style="display:none; position:absolute;">
										<?//=DivContentLoad('div_employee_size','employee_size','name',TABLE_EMPLOYEE_SIZE)?>
									</div>
								</td>
							</tr>
						</table>-->					  </td>
						...
						<td align="left" valign="top"><table width="490" border="0" align="center" cellpadding="2" cellspacing="0">
							<tr>
								...
								<!--<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="my_div">
									<tr>
									  <td valign="top">
											<input type="text" name="delivery_schedule" id="delivery_schedule" class="text-new-227" style="margin-top:0px; border:0px solid #000000; color:#737373;" onfocus="fieldHighlightCombo('delivery_schedule');MyCombo_Open('div_delivery_schedule');AllComboDivCloseAlert('div_delivery_schedule');" size="34" value="<? if($delivery_schedule==''){echo 'Any';}else{echo $delivery_schedule;}?>" onblur="fieldLosslightCombo('delivery_schedule');"/>
											<img src="images/down.png" align="right" width="20" height="20" onclick="MyCombo_Open('div_delivery_schedule');"/>								</td>
									</tr>
									<tr>
										<td valign="top">
											<div id="div_delivery_schedule" class="inner" style="display:none; position:absolute;">
												<?//=DivContentLoad('div_delivery_schedule','delivery_schedule','name',TABLE_EMAIL_UPDATE)?>
											</div>
										</td>
									</tr>
								</table>-->
							...
							<tr>
							  ...
							  <td width="270"  align="left" valign="top">
								  ...
								  <!--<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" class="my_div">
									<tr>
									  <td valign="top">
											<input type="text" name="monthly_budget" id="monthly_budget" class="text-new-227" style="margin-top:0px; border:0px solid #000000; color:#737373;" onfocus="fieldHighlightCombo('monthly_budget');MyCombo_Open('div_monthly_budget');AllComboDivCloseAlert('div_monthly_budget');" size="34" value="<? if($monthly_budget==''){echo 'Any';}else{echo $monthly_budget;}?>" onblur="fieldLosslightCombo('monthly_budget');"/>
											<img src="images/down.png" align="right" width="20" height="20" onclick="MyCombo_Open('div_monthly_budget');"/>								</td>
									</tr>
									<tr>
										<td valign="top">
											<div id="div_monthly_budget" class="inner" style="display:none; position:absolute;">
												<?//=DivContentLoadPrice('div_monthly_budget','monthly_budget','ap_name',TABLE_ALERT_PRICE)?>
											</div>
										</td>
									</tr>
								  </table>-->
				  ...
				  <td align="center" valign="middle">
						<table width="214" border="0" align="center" cellpadding="0" cellspacing="0">
							<tr>
							  ...
							  <form name="frmAlertBack" id="frmAlertBack" method="post" action="delete-user.php?action=CreateAlert&selected_menu=user&uID=<?=$uID?>">
