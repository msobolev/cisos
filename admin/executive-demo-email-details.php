...
$sql_query = "select * from " . TABLE_EXECUTIVE_DEMO_EMAIL_INFO." order by add_date desc";

/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'executive-demo-email-details.php';
...
if($action=='DemoEmailCode' || $action == 'DemoEmailShow'){
	...
	$emailCode = com_db_output(com_db_Getvalue("select sent_email from ".TABLE_EXECUTIVE_DEMO_EMAIL_INFO." where info_id='".$info_id."'"));
}
if($action=='delete'){
	com_db_query("delete from " . TABLE_EXECUTIVE_DEMO_EMAIL_INFO . " where info_id = '" . $mID . "'");
	com_redirect("executive-demo-email-details.php?p=" . $p . "&selected_menu=demoemail&msg=" . msg_encode("Demo Email deleted successfully"));
}
...
<script type="text/javascript" language="javascript">
	...
	if (agree)
		window.location = "executive-demo-email-details.php?selected_menu=demoemail&mID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "executive-demo-email-details.php?selected_menu=demoemail&mID=" + nid + "&p=" + p ;
	}
</script>
 <tr>
...
<?php if(($action == '') || $action == 'save'){	?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          ...
          <tr>
			...
			<form name="topicform" action="executive-demo-email-details.php?action=DemoEmailCreate&selected_menu=demoemail" method="post">
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="executive-demo-email-details.php?action=DemoEmailShow&selected_menu=demoemail&emailid=<?=$data_sql['info_id'];?>">Demo Email Show</a></td>
                <td height="30" align="left" valign="middle" class="right-border-text"><a href="executive-demo-email-details.php?action=DemoEmailCode&selected_menu=demoemail&emailid=<?=$data_sql['info_id'];?>">Demo email code copy for email</a></td>
                <td height="30" align="left" valign="middle" class="right-border-text"><a href="<?=HTTPS_SITE_URL."executive-demo-email-show.php?emailid=".$data_sql['email_id'];?>" target="_blank">Details form website</a></td>
...
<?php }elseif($action=='DemoEmailShow'){ ?>
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		 ...
		 <!--start iner table  -->
			...
			<tr>
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='executive-demo-email-details.php?selected_menu=demoemail'" /></td>
			</tr>
