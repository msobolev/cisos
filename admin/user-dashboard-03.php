...
$TodaySendAlert = com_db_GetValue("select count(distinct(user_id)) from " . TABLE_ALERT_SEND_INFO."  where DATE_FORMAT(FROM_UNIXTIME(`sent_date`), '%Y-%m-%d')='".date("Y-m-d")."'");
$LastweekSendAlert = com_db_GetValue("select count(distinct(user_id)) from " . TABLE_ALERT_SEND_INFO."  where DATE_FORMAT(FROM_UNIXTIME(`sent_date`), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(`sent_date`), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-6,date("Y")))."'");
$LastmonthSendAlert = com_db_GetValue("select count(distinct(user_id)) from " . TABLE_ALERT_SEND_INFO."  where DATE_FORMAT(FROM_UNIXTIME(`sent_date`), '%Y-%m-%d') <='".date("Y-m-d")."' and DATE_FORMAT(FROM_UNIXTIME(`sent_date`), '%Y-%m-%d') >='".date("Y-m-d",mktime(0,0,0,date("m"),date("d")-30,date("Y")))."'");
