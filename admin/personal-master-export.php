...
if($action == 'PersonalDataExport'){
	...
	$download_query_string = "select * from ".TABLE_PERSONAL_MASTER; 
		...
		if($fname==''){
			...
			if($from_date!='' && $to_date !=''){
				$download_query = $download_query_string ."	where status='0' and first_name like '".$fname."%' and add_date >='".$from_date."' and add_date <='".$to_date."'";
			}else{
				$download_query = $download_query_string ."	where status='0' and first_name like '".$fname."%'";
			}
		}
		...
		while($download_row=com_db_fetch_array($result)) {
			...
			if($edu_ugrad_year==0){$edu_ugrad_year='';}
			...
			if($edu_grad_year==0){$edu_grad_year='';}
      ...
      <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <form name="exportfile" method="post" action="personal-master-export.php?action=PersonalDataExport">
			...
			<tr><td colspan="2" height="20" class="page-text">&nbsp;&nbsp;&nbsp;<b>Export Records start with first name alphabet</b></td></tr>
			  <tr>
