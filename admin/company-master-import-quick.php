...
{
	$industry_tbl_arr[$industry_row['industry_id']] = trim($industry_row['title']);
}
//echo "<pre>";	print_r($industry_tbl_arr);	echo "</pre>"; die();
...
//echo "<br>One ID: ".array_search("Non-profit",$industry_tbl_arr,true);
//echo "<br>Two ID: ".array_search("Advocacy Organizations",$industry_tbl_arr,true);
//die();
								...
								if(sizeof($newArray)== 21){
									...
									foreach($newArray as $key => $value){
										...
										}elseif($p==1)
										{
											...
											if(strpos($import_revenue_type,">$1B") > -1)
											{
												$import_revenue_type = str_replace(">$1B","> $1 Billion",$import_revenue_type);		
											}
											elseif(strpos($import_revenue_type,"B") > -1)
												$import_revenue_type = str_replace("B"," Billion",$import_revenue_type);	
											elseif(strpos($import_revenue_type,"M") > -1)
											...
											if(sizeof($tot_ind)==2){
												...
												$tot_ind[0] = trim($tot_ind[0]);
												$tot_ind[1] = trim($tot_ind[1]);
												
												//echo "<br>FA Zero index:".$tot_ind[0].":";
												//echo "<br>FA One index : ".$tot_ind[1];
												//echo "<pre>FA industry_tbl_arr: ";	print_r($industry_tbl_arr);	echo "</pre>";
												
											
												//$ind_group_id = com_db_GetValue("select industry_id from " . TABLE_INDUSTRY . " where title = '".str_replace(';',',',trim($tot_ind[0]))."'");
												if(strpos($tot_ind[0],';') > -1)
													$ind_group_id = array_search(str_replace(';',',',trim($tot_ind[0])),$industry_tbl_arr,true);
												else	
													$ind_group_id = array_search(trim($tot_ind[0]),$industry_tbl_arr,true);
												
												//echo "<br>FA ind_group_id: ".$ind_group_id;
												
												//$industry_id = com_db_GetValue("select industry_id from " . TABLE_INDUSTRY . " where title = '".str_replace(';',',',trim($tot_ind[1]))."'");
												
												if(strpos($tot_ind[1],';') > -1)
													$industry_id = array_search(str_replace(';',',',trim($tot_ind[1])),$industry_tbl_arr,true);
												else	
													$industry_id = array_search(trim($tot_ind[1]),$industry_tbl_arr,true);
												//echo "<br>FA industry_id: ".$industry_id;
												
												$industry_name = com_db_output(str_replace(';',',', trim($tot_ind[1])));
												$industry_group_name = com_db_output(str_replace(';',',', trim($tot_ind[0])));
												if($ind_group_id =='' ){
													//echo "<br>FA Within IF";
													$industry_invalid = 1;
												}elseif($ind_group_id !='' && $industry_id ==''){
													
													//echo "<br>FA Within ELSE IF";
											...
											if(strlen($import_zip_code) == 0 || strlen($import_zip_code) == 5 || $import_country == 'CANADA')
											{
											...
											if(strlen($import_phone) > 12)
												$import_phone = substr($import_phone,0,12);
    ...
    <td align="left" valign="top" bgcolor="#FFFFFF"><table width="975" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <form name="importfile" method="post" action="company-master-import-quick.php?selected_menu=master&action=save" enctype="multipart/form-data" onsubmit="return checkForm();">	
