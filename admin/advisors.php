...
$sql_query = "select * from " . TABLE_TEAM_ADVISORS . " where type='Advisors' order by ta_id desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'advisors.php';
...
$tID = (isset($_GET['tID']) ? $_GET['tID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_TEAM_ADVISORS . " where ta_id = '" . $tID . "'");
		 	com_redirect("advisors.php?p=" . $p . "&selected_menu=team&msg=" . msg_encode("Advisors deleted successfully"));
	  ...
	  case 'edit':
		...
		case 'editsave':
			...
			$query = "update " . TABLE_TEAM_ADVISORS . " set name = '" . $name . "',  details = '" . $details . "' where ta_id = '" . $tID . "'";
			com_db_query($query);
			...
			if($image!=''){
	  		...
	  		com_redirect("advisors.php?p=". $p ."&tID=" . $tID . "&selected_menu=team&msg=" . msg_encode("Advisors update successfully"));
	...
	case 'addsave':
			...
			$query = "insert into " . TABLE_TEAM_ADVISORS . " (name, details, type, add_date, status) values ('$name', '$details', 'Advisors', '$added', '0')";
			com_db_query($query);
			...
			if($image!=''){
	  	 	...
	  	 	com_redirect("advisors.php?p=" . $p . "&selected_menu=team&msg=" . msg_encode("Advisors added successfully"));
...
function confirm_del(nid,p){
	var agree=confirm("Advisors will be deleted. \n Do you want to continue?");
	if (agree)
		window.location = "advisors.php?selected_menu=team&tID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "advisors.php?selected_menu=team&tID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "advisors.php?selected_menu=team";
	}	
...
function confirm_artivate(nid,p,status){
	if(status=='1'){
		var msg="Advisors will be active. \n Do you want to continue?";
	}else{
		var msg="Advisors will be Inactive. \n Do you want to continue?";
	}
	var agree=confirm(msg);
	if (agree)
		window.location = "advisors.php?selected_menu=team&tID=" + nid + "&p=" + p + "&status=" + status + "&action=activate";
	else
		window.location = "advisors.php?selected_menu=team&tID=" + nid + "&p=" + p ;
}
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="30%" align="left" valign="middle" class="heading-text">Advisors Manager</td>
                  <td width="34%" align="left" valign="middle" class="message"><?=$msg?></td>
                  <? if($btnAdd=='Yes'){ ?>
                  <td width="8%" align="right" valign="middle"><a href="#"><img src="images/add-icon.jpg" border="0" width="25" height="28" alt="Add Advisors" name="Add Advisors" onclick="window.location='advisors.php?action=add&selected_menu=team'"  /></a></td>
                  <td width="8%" align="left" valign="middle" class="nav-text">Add New </td>
                  <? }
				  if($btnDelete=='Yes'){ ?>
                  <td width="4%" align="right" valign="middle"><a href="#"><img src="images/delete-icon.jpg" border="0" width="25" height="28"  alt="Delete Advisors" name="Delete Advisors" onclick="AllDelete('<?=$numRows?>');"  /></a></td>
          ...
          <tr>
			...
			<form name="topicform" action="advisors.php?action=alldelete&selected_menu=team" method="post">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
			  <tr>
				...
				<td width="543" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Name &amp; Designation</span> </td>
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="advisors.php?action=detailes&selected_menu=team&tID=<?=$data_sql['ta_id'];?>"><?=com_db_output($data_sql['name'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
					  ...
					  <? if($btnStatus=='Yes'){
						...
						if($btnEdit=='Yes'){ ?>
						<td width="23%" align="center" valign="middle"><a href="#"><img src="images/small-edit-icon.gif" width="16" height="16" alt="Edit" name="Edit" border="0" onclick="window.location='advisors.php?selected_menu=team&p=<?=$p;?>&tID=<?=$data_sql['ta_id'];?>&action=edit'" /></a><br />
						  Edit</td>
                       <? } 
						 if($btnDelete=='Yes'){ ?>  
...
<?php } elseif($action=='edit'){ ?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Advisors Manager :: Edit Advisors </td>
		 ...
		 <!--start iner table  -->
			...
			<form name="DateTest" method="post" action="advisors.php?action=editsave&selected_menu=team&tID=<?=$tID;?>&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
			<tr>
            ...
            <tr>
			  ...
			  <td width="78%" align="left" valign="top">
                ...
                echo '<img src="../team_photo/thumb/'.$image_path.'" title="Advisors Photo" alt="Advisors Photo" /><br /><br />';
                 } ?>
		  ...
		  <!---------------------------->
			...
			<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><div id="butt" style="display:none;"><input type="submit" value="Update Advisors" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='advisors.php?p=<?=$p;?>&tID=<?=$tID;?>&selected_menu=team'" /></div></td></tr>
			</form>
			</table>
		 <!-- end inner table -->
		  </td>
...
function chk_form(){
	...
	return false;
          ...
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Advisors Manager :: Add Advisors </td>
				  <td width="50%" valign="middle" class="message" align="left"><?=$msg?></td>
		 ...
		 <!--start iner table  -->
		  <table width="95%" align="left" cellpadding="5" cellspacing="5" border="0">
		<form name="DateTest" method="post" action="advisors.php?action=addsave&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
		<tr>
      ...
      <!---------------------------->
		...
		<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><div id="butt" style="display:none;"><input type="submit" value="Add Advisors" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='advisors.php?p=<?=$p;?>&tID=<?=$tID;?>&selected_menu=team'" /></div></td></tr>
		</form>
		</table>
		 <!-- end inner table -->
		  </td>
...
?>		
  	  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Advisors Manager :: Advisors Details </td>
				  <td width="50%" valign="middle" class="message" align="left"><?=$msg?></td>
		 ...
		 <!--start iner table  -->
			...
			<tr>
				...
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='advisors.php?p=<?=$p;?>&tID=<?=$tID;?>&selected_menu=team'" /></td>
			</tr>
