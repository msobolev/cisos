...
$sql_query = "select * from " . TABLE_SITEMAP_PERSONAL . " order by sitemap_id desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'create-sitemap-personal.php';
...
$fID = (isset($_GET['fID']) ? $_GET['fID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
	   		$file_name = com_db_GetValue("select sitemap_name from " . TABLE_SITEMAP_PERSONAL . " where sitemap_id = '" . $fID . "'");
			if(file_exists("../".$file_name)){
				unlink("../".$file_name);
			}
			com_db_query("delete from " . TABLE_SITEMAP_PERSONAL . " where sitemap_id = '" . $fID . "'");
		 	com_redirect("create-sitemap-personal.php?p=" . $p . "&selected_menu=sitemap&msg=" . msg_encode("Sitemap deleted successfully"));
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($sitemap_id) ; $i++){
				$file_name = com_db_GetValue("select sitemap_name from " . TABLE_SITEMAP_PERSONAL . " where sitemap_id = '" . $sitemap_id[$i] . "'");
				if(file_exists("../".$file_name)){
					unlink("../".$file_name);
				}
				com_db_query("delete from " . TABLE_SITEMAP_PERSONAL . " where sitemap_id = '" . $sitemap_id[$i] . "'");
			}
		 	com_redirect("create-sitemap-personal.php?p=" . $p . "&selected_menu=sitemap&msg=" . msg_encode("Sitemap deleted successfully"));
	...
	case 'addsave':
			com_db_query("delete from " . TABLE_SITEMAP_PERSONAL . " where 1");
			$added=date('Y-m-d');
			...
			//main sitemap create
			$url_query = "select sitemap_name from " .TABLE_SITEMAP_PERSONAL ;
			$url_result = com_db_query($url_query);
			if($url_result){
	  		...
	  		com_redirect("create-sitemap-personal.php?p=" . $p . "&selected_menu=sitemap&msg=" . msg_encode("Sitemap create successfully"));
		break;	
...
function confirm_del(nid,p){
	...
	if (agree)
		window.location = "create-sitemap-personal.php?selected_menu=sitemap&fID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "create-sitemap-personal.php?selected_menu=sitemap&fID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "create-sitemap-personal.php?selected_menu=sitemap";
	}	
...
<?php if(($action == '') || ($action == 'save')){	?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td width="8%" align="right" valign="middle"><a href="#"><img src="images/add-icon.jpg" border="0" width="25" height="28" alt="Add Sitemap" question="Add Sitemap" onclick="window.location='create-sitemap-personal.php?action=addsave&selected_menu=sitemap'"  /></a></td>
          ...
          <tr>
			...
			<form name="topicform" action="create-sitemap-personal.php?action=alldelete&selected_menu=sitemap" method="post">
