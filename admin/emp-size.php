...
$sql_query = "select * from " . TABLE_EMPLOYEE_SIZE . " order by id desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'emp-size.php';
...
$pID = (isset($_GET['pID']) ? $_GET['pID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_EMPLOYEE_SIZE . " where id = '" . $pID . "'");
		 	com_redirect("emp-size.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Employee Size deleted successfully"));
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($id) ; $i++){
				com_db_query("delete from " . TABLE_EMPLOYEE_SIZE . " where id = '" . $id[$i] . "'");
			}
		 	com_redirect("emp-size.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Employee Size deleted successfully"));
	  ...
	  case 'edit':
	   		...
	   		$query_edit=com_db_query("select * from " . TABLE_EMPLOYEE_SIZE . " where id = '" . $pID . "'");
		...
		case 'editsave':
			...
			$query = "update " . TABLE_EMPLOYEE_SIZE . " set name = '" . $name . "', from_range ='".$from_range."', to_range = '".$to_range."', modify_date = '".$modify_date."' where id = '" . $pID . "'";
			com_db_query($query);
			
			com_redirect("emp-size.php?p=". $p ."&pID=" . $pID . "&selected_menu=others&msg=" . msg_encode("Employee Size update successfully"));
	...
	case 'addsave':
			...
			$query = "insert into " . TABLE_EMPLOYEE_SIZE . " (name, from_range,to_range, add_date, status) values ('$name', '$from_range','$to_range', '$add_date', '0')";
			com_db_query($query);
			$insert_id = com_db_insert_id();
			
			com_redirect("emp-size.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Employee Size added successfully"));
	...
	case 'activate':
	   		...
	   		$status=$_GET['status'];
			if($status==0){
				$query = "update " . TABLE_EMPLOYEE_SIZE . " set status = '1' where id = '" . $pID . "'";
			}else{
				$query = "update " . TABLE_EMPLOYEE_SIZE . " set status = '0' where id = '" . $pID . "'";
			}	
			com_db_query($query);
	  		com_redirect("emp-size.php?p=". $p ."&pID=" . $pID . "&selected_menu=others&msg=" . msg_encode("Employee Size update successfully"));
...
function confirm_del(nid,p){
	var agree=confirm("Employee Size will be deleted. \n Do you want to continue?");
	if (agree)
		window.location = "emp-size.php?selected_menu=others&pID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "emp-size.php?selected_menu=others&pID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "emp-size.php?selected_menu=others";
	}	
...
function confirm_artivate(nid,p,status){
	if(status=='1'){
		var msg="Employee Size will be active. \n Do you want to continue?";
	}else{
		var msg="Employee Size will be Inactive. \n Do you want to continue?";
	}
	var agree=confirm(msg);
	if (agree)
		window.location = "emp-size.php?selected_menu=others&pID=" + nid + "&p=" + p + "&status=" + status + "&action=activate";
	else
		window.location = "emp-size.php?selected_menu=others&pID=" + nid + "&p=" + p ;
}
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="30%" align="left" valign="middle" class="heading-text">Employee Size Manager</td>
                  <td width="34%" align="left" valign="middle" class="message"><?=$msg?></td>
                  <? if($btnAdd=='Yes'){ ?>
                  <td width="8%" align="right" valign="middle"><a href="#"><img src="images/add-icon.jpg" border="0" width="25" height="28" alt="Add Employee Size" name="Add Employee Size" onclick="window.location='emp-size.php?action=add&selected_menu=others'"  /></a></td>
                  <td width="8%" align="left" valign="middle" class="nav-text">Add New </td>
                  <? }
				  if($btnDelete=='Yes'){ ?>
                  <td width="4%" align="right" valign="middle"><a href="#"><img src="images/delete-icon.jpg" border="0" width="25" height="28"  alt="Delete Employee Size" name="Delete Employee Size" onclick="AllDelete('<?=$numRows?>');"  /></a></td>
          ...
          <tr>
			...
			<form name="topicform" action="emp-size.php?action=alldelete&selected_menu=others" method="post">
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><a href="emp-size.php?action=detailes&selected_menu=others&pID=<?=$data_sql['id'];?>"><?=com_db_output($data_sql['name'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
					  ...
					  <? if($btnStatus=='Yes'){ 	
					   ...
					   if($btnEdit=='Yes'){ ?>
						<td width="23%" align="center" valign="middle"><a href="#"><img src="images/small-edit-icon.gif" width="16" height="16" alt="Edit" name="Edit" border="0" onclick="window.location='emp-size.php?selected_menu=others&p=<?=$p;?>&pID=<?=$data_sql['id'];?>&action=edit'" /></a><br />
						  Edit</td>
                       <? }
					    if($btnDelete=='Yes'){ ?>   
...
<?php } elseif($action=='edit'){ ?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Employee Size Manager :: Edit </td>
		 ...
		 <!--start iner table  -->
			...
			<form name="DateTest" method="post" action="emp-size.php?action=editsave&selected_menu=others&pID=<?=$pID;?>&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
			<tr>
			...
			<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><input type="submit" value="Update Employee Size" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='emp-size.php?p=<?=$p;?>&pID=<?=$pID;?>&selected_menu=others'" /></td></tr>
			</form>
			</table>
		 <!-- end inner table -->
		  </td>
...
function chk_form(){
	...
	return false;
          ...
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Employee Size Manager :: Add</td>
				  <td width="50%" valign="middle" class="message" align="left"><?=$msg?></td>
		 ...
		 <!--start iner table  -->
		  <table width="95%" align="left" cellpadding="5" cellspacing="5" border="0">
		<form name="DateTest" method="post" action="emp-size.php?action=addsave&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
		<tr>
          <td width="15%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;From Range:</td>
          <td width="85%" align="left" valign="top">
		...
		<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><input type="submit" value="Add Employee Size" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='emp-size.php?p=<?=$p;?>&pID=<?=$pID;?>&selected_menu=others'" /></td></tr>
		</form>
		</table>
		 <!-- end inner table -->
		  </td>
...
?>		
  	  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="50%" align="left" valign="middle" class="heading-text">Employee Size Manager ::  Details </td>
				  <td width="50%" valign="middle" class="message" align="left"><?=$msg?></td>
		 ...
		 <!--start iner table  -->
			...
			<tr>
				...
				<td align="left" valign="top"><input type="button" class="submitButton" value="Back" onclick="window.location='emp-size.php?p=<?=$p;?>&pID=<?=$pID;?>&selected_menu=others'" /></td>
			</tr>
