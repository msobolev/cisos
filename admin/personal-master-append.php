				...
				if(is_uploaded_file($data_file)){
					...
					if($get_ext=='csv'){
						...
						if(move_uploaded_file($data_file , $destination_file)){
							...
							//echo "<br>FA ";	print_r($filecontents);	echo "</pre>";
							//caption start
							...
							//$col_fld_cap = array('company_name','company_website','company_logo','company_revenue','company_employees','company_industry','email_pattern','leadership_page','address','address2','city','country','state','zip_code','phone','fax','about_company','facebook_link','linkedin_link','twitter_link','googleplush_link');
							$col_fld_cap = array('status','personal_id','first_name','middle_name','last_name','email','email_verified','email_verified_date','phone','lin_url','facebook_link','linkedin_link','twitter_link','googleplush_link','about_person','edu_ugrad_degree','edu_ugrad_specialization','edu_ugrad_college','edu_ugrad_year','edu_grad_degree','edu_grad_specialization','edu_grad_college','edu_grad_year','personal','awards_title','awards_given_by','awards_date','add_date','modify_date','create_by','person_tigger_name','email_validation_api_result');	
							// FAR - 6 April
							$col_fld_cap = array('status','personal_id','first_name','middle_name','last_name','email','verified_image','email_verified','email_verified_date','phone','lin_url','personal_image','facebook_link','linkedin_link','twitter_link','googleplush_link','about_person','edu_ugrad_degree','edu_ugrad_specialization','edu_ugrad_college','edu_ugrad_year','edu_grad_degree','edu_grad_specialization','edu_grad_college','edu_grad_year','personal','awards_title','awards_given_by','awards_date','add_date','modify_date','create_by','person_tigger_name','email_validation_api_result');	
							
							$capnewString = $filecontents[0];
							$capnewArray = explode(',', $capnewString);
							
							$id_col = "personal_id";//$capnewArray[0];
							...
							else
							{
								...
								//com_redirect("personal-master-append.php?selected_menu=master&msg=" . msg_encode($msg));	
							}
							...
							for($i=$columnheadings; $i<sizeof($filecontents); $i++) {
								...
								if(sizeof($newArray)== 2){
									...
									$strQuery = "UPDATE " . TABLE_PERSONAL_MASTER . " SET ";
    ...
    <td align="left" valign="top" bgcolor="#FFFFFF"><table width="975" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <form name="importfile" method="post" action="personal-master-append.php?selected_menu=master&action=save" enctype="multipart/form-data" onsubmit="return checkForm();">	
			<table width="60%" border="0" cellspacing="0" cellpadding="5">
			  <tr>
			  	<td colspan="2" height="20" align="right" class="page-text"><a href="download-personal-status.php?type=Sample">Sample CSV Download</a></td></tr>
			  <tr>
			  <tr>
