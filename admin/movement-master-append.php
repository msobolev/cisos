				...
				if(is_uploaded_file($data_file)){
					...
					if($get_ext=='csv'){
						...
						if(move_uploaded_file($data_file , $destination_file)){
							...
							// Todo - add one missing column from csv file ie Contact URL
							//$col_fld_cap = array('first_name','middle_name','last_name','title','headline','company_name','company_website','company_industry','address','city','country','state','zip_code','phone','email','announce_date','effective_date','movement_type','full_body','link','company_employee','company_revenue','source_id','short_url','what_happened','about_person','about_company','more_link','not_current');
							$cols_personal_master = array('first_name','middle_name','last_name','email','about_person');							
							$cols_company_master = array('company_name','company_website','company_industry','address','city','country','state','zip_code','phone','company_employee','company_revenue','about_company');														
							$cols_movement_master = array('title','headline','announce_date','effective_date','movement_type','full_body','link','source_id','short_url','what_happened','more_link','not_current');								
							$capnewString = $filecontents[0];
							$capnewArray = explode(',', $capnewString);
							
							$id_col = trim($capnewArray[0]);	//"company_id";//$capnewArray[0];
							$col = trim($capnewArray[1]);
							//echo "<br>ID COL: ".$id_col;
							//echo "<br>COL: ".$col;
							if($id_col == 'company_id')
							{
								$update_table = TABLE_COMPANY_MASTER;
									
								if (in_array($col, $cols_company_master)) 
								{
								}
								else
								{ 
									$msg = "Field in csv file doesn't mached with any column of DB table.";
									com_redirect("movement-master-append.php?selected_menu=master&msg=" . msg_encode($msg));	
								}
							}
							elseif($id_col == 'personal_id')
							{
								

								$update_table = TABLE_PERSONAL_MASTER;
								if (in_array($col, $cols_personal_master)) 
								{
								}
								else
								{ 
							...
							elseif($id_col == 'move_id')
							{
								
								$update_table = TABLE_MOVEMENT_MASTER;
								if (in_array($col, $cols_movement_master)) 
								{
								}
								else
								{
							...
							else
							{
								...
								$msg = "Column ID field in csv file doesn't mached with any column of DB table.";
								com_redirect("movement-master-append.php?selected_menu=master&msg=" . msg_encode($msg));
							}
							...
							*/
							...
							for($i=$columnheadings; $i<sizeof($filecontents); $i++) {
								...
								if(sizeof($newArray)== 2){
									...
									$strQuery = "UPDATE " . $update_table . " SET ";
									...
									//foreach($newArray as $key => $value){
										...
										//echo "<br><br><br><br>Str Q: ".$companyQuery; die();
    ...
    <td align="left" valign="top" bgcolor="#FFFFFF"><table width="975" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  ...
		  <form name="importfile" method="post" action="movement-master-append.php?selected_menu=master&action=save" enctype="multipart/form-data" onsubmit="return checkForm();">	
			<table width="60%" border="0" cellspacing="0" cellpadding="5">
			  <tr>
			  	<td colspan="2" height="20" align="right" class="page-text"><a href="download-company-append-info.php?type=Sample">Sample CSV Download</a></td></tr>
			  <tr>
			  <tr>
