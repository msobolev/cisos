...
$sql_query = "select * from " . TABLE_POTENTIAL_CLIENTS . " order by image_id desc";
/***************** FOR PAGIN ***********************/
$starting_point = $items_per_page * ($p - 1);
/****************** END PAGIN ***********************/
$main_page = 'potential-clients.php';
...
$pID = (isset($_GET['pID']) ? $_GET['pID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_POTENTIAL_CLIENTS . " where image_id = '" . $pID . "'");
		 	com_redirect("potential-clients.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Image deleted successfully"));
	  ...
	  case 'alldelete':
			...
			for($i=0; $i< sizeof($image_id) ; $i++){
				com_db_query("delete from " . TABLE_POTENTIAL_CLIENTS . " where image_id = '" . $image_id[$i] . "'");
			}
		 	com_redirect("potential-clients.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Image deleted successfully"));
	  ...
	  case 'edit':
	   		...
	   		$query_edit=com_db_query("select * from " . TABLE_POTENTIAL_CLIENTS . " where image_id = '" . $pID . "'");
			...
			$site_url = com_db_output($data_edit['site_url']);
		...
		case 'editsave':
			...
			$site_url=com_db_input($_POST['site_url']);
			$modify_date = date('Y-m-d');
			
			$query = "update " . TABLE_POTENTIAL_CLIENTS . " set caption = '" . $caption . "',site_url='".$site_url."' where image_id = '" . $pID . "'";
			com_db_query($query);
			...
			if($image!=''){
				if(is_uploaded_file($image)){
					...
					if(strtolower($get_ext)=="jpg" || strtolower($get_ext)=="gif" || strtolower($get_ext)=="jpge" || strtolower($get_ext)=="png"){
						...
						$destination_image = '../clients_logo/' . $org_image_name;
						if(move_uploaded_file($image , $destination_image)){
							$query = "UPDATE " . TABLE_POTENTIAL_CLIENTS . " SET image_path = '" . $org_image_name . "' WHERE image_id = '" . $pID ."'";
							com_db_query($query);
						}
					}	
				}	
			...
			com_redirect("potential-clients.php?p=". $p ."&pID=" . $pID . "&selected_menu=others&msg=" . msg_encode("Image update successfully"));
	...
	case 'addsave':
			...
			$query = "insert into " . TABLE_POTENTIAL_CLIENTS . " (caption, site_url, add_date, status) values ('$caption', '$site_url', '$add_date', '0')";
			com_db_query($query);
			...
			if($image!=''){
				if(is_uploaded_file($image)){
					...
					if(strtolower($get_ext)=="jpg" || strtolower($get_ext)=="gif" || strtolower($get_ext)=="jpge" || strtolower($get_ext)=="png"){
						...
						$destination_image = '../clients_logo/' . $org_image_name;
						if(move_uploaded_file($image , $destination_image)){
							$query = "UPDATE " . TABLE_POTENTIAL_CLIENTS . " SET image_path = '" . $org_image_name . "' WHERE image_id = '" . $insert_id ."'";
							com_db_query($query);
						}
					}	
				}	
			...
			com_redirect("potential-clients.php?p=" . $p . "&selected_menu=others&msg=" . msg_encode("Image added successfully"));
	...
	case 'activate':
	   		...
	   		$status=$_GET['status'];
			if($status==0){
				$query = "update " . TABLE_POTENTIAL_CLIENTS . " set status = '1' where image_id = '" . $pID . "'";
			}else{
				$query = "update " . TABLE_POTENTIAL_CLIENTS . " set status = '0' where image_id = '" . $pID . "'";
			}	
			com_db_query($query);
	  		com_redirect("potential-clients.php?p=". $p ."&pID=" . $pID . "&selected_menu=others&msg=" . msg_encode("Image update successfully"));
...
function confirm_del(nid,p){
	...
	if (agree)
		window.location = "potential-clients.php?selected_menu=others&pID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "potential-clients.php?selected_menu=others&pID=" + nid + "&p=" + p ;
}
...
function AllDelete(numRows){
	...
	if(flg==0){
		...
		if (agree)
			...
			window.location = "potential-clients.php?selected_menu=others";
	}	
...
function confirm_artivate(nid,p,status){
	...
	if (agree)
		window.location = "potential-clients.php?selected_menu=others&pID=" + nid + "&p=" + p + "&status=" + status + "&action=activate";
	else
		window.location = "potential-clients.php?selected_menu=others&pID=" + nid + "&p=" + p ;
}
...
<?php if(($action == '') || ($action == 'save')){	?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
          <tr>
            <td align="center" valign="middle" class="right"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <? if($btnAdd=='Yes'){ ?>
                  <td width="8%" align="right" valign="middle"><a href="#"><img src="images/add-icon.jpg" border="0" width="25" height="28" alt="Add Image" caption="Add Image" onclick="window.location='potential-clients.php?action=add&selected_menu=others'"  /></a></td>
          ...
          <tr>
			...
			<form caption="topicform" action="potential-clients.php?action=alldelete&selected_menu=others" method="post">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
			  <tr>
				...
				<td width="161" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Image</span> </td>
				<td width="247" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Caption</span> </td>
				<td width="200" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Site URL</span> </td>
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text"><img src="../clients_logo/<?=com_db_output($data_sql['image_path'])?>" /></td>
				<td height="30" align="left" valign="middle" class="right-border-text"><?=com_db_output($data_sql['caption'])?></a></td>
				<td height="30" align="left" valign="middle" class="right-border-text"><?=com_db_output($data_sql['site_url'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
					  ...
					  <? if($btnStatus=='Yes'){	
					   ...
					   if($btnEdit=='Yes'){ ?>
						<td width="23%" align="center" valign="middle"><a href="#"><img src="images/small-edit-icon.gif" width="16" height="16" alt="Edit" caption="Edit" border="0" onclick="window.location='potential-clients.php?selected_menu=others&p=<?=$p;?>&pID=<?=$data_sql['image_id'];?>&action=edit'" /></a><br />
						  Edit</td>
                      <? }
                       if($btnDelete=='Yes'){ ?>   
...
<?php } elseif($action=='edit'){ ?>			
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		 ...
		 <!--start iner table  -->
			...
			<form caption="DateTest" method="post" action="potential-clients.php?action=editsave&selected_menu=others&pID=<?=$pID;?>&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
			<tr>
			...
			<tr>
			  <td width="11%" align="left" class="page-text" valign="top">&nbsp;Site URL:</td>
			  <td width="89%" align="left" valign="top">
				<input type="text" name="site_url" id="site_url" size="40" value="<?=$site_url;?>" /></td>	
			</tr>
			<tr>
			  <td width="11%" align="left" class="page-text" valign="top">&nbsp;Image:</td>
			  <td width="89%" align="left" valign="top">
			  	<img src="../clients_logo/<?=$image_path?>" /><br /><br />
				<input type="file" name="image" id="image" size="40" />
				<p>Height = 40px, extention must be .gif, .jpg, .jpge, .png </p></td>	
			</tr>
			<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><input type="submit" value="Update Image" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='potential-clients.php?p=<?=$p;?>&pID=<?=$pID;?>&selected_menu=others'" /></td></tr>
			</form>
			</table>
		 <!-- end inner table -->
		  </td>
...
function chk_form(){
	...
	return false;
		 ...
		 <!--start iner table  -->
		  <table width="95%" align="left" cellpadding="5" cellspacing="5" border="0">
		<form caption="DateTest" method="post" action="potential-clients.php?action=addsave&p=<?=$p;?>" enctype="multipart/form-data" onsubmit="return chk_form();">
			<tr>
			  <td width="11%" align="left" class="page-text" valign="top">&nbsp;Caption:</td>
			  <td width="89%" align="left" valign="top">
			...
			<tr>
			  <td width="11%" align="left" class="page-text" valign="top">&nbsp;Site URL:</td>
			  <td width="89%" align="left" valign="top">
				<input type="text" name="site_url" id="site_url" size="40" /></td>	
			</tr>
			<tr>
			  <td width="11%" align="left" class="page-text" valign="top">&nbsp;Image:</td>
			  <td width="89%" align="left" valign="top">
		...
		<tr><td align="left" valign="top">&nbsp;</td><td align="left" valign="top"><input type="submit" value="Add Image" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="cancel" onclick="window.location='potential-clients.php?p=<?=$p;?>&pID=<?=$pID;?>&selected_menu=others'" /></td></tr>
		</form>
		</table>
		 <!-- end inner table -->
		  </td>
