...
if($action == 'UserLogin'){
	...
	$user_query = "select * from " . TABLE_USER ." where LOWER(`email`) = '".strtolower($login_email)."' and status='0'";
	...
	if($user_result){
		...
		if($row_count > 0){
			...
			if(strtoupper($login_pass) == strtoupper($password_db)){	
				...
				session_register('sess_is_user','sess_user_id' , 'sess_username', 'sess_payment');
				...
				$sql_login_query = "insert into " . TABLE_LOGIN_HISTORY . " (user_id,add_date,login_time,logout_time,log_status) values('$user_id','$add_date','$login_time','$logout_time','$log_status')";
...
if($action =='ChoosePricing'){
	...
	session_register('sess_sub_choose' , 'sess_alert_price');
