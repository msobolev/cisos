...
if($action == 'back'){
	$reg_id = $_REQUEST['resID'];
	$res_result = com_db_query("select * from " . TABLE_USER . " where user_id = '".$reg_id."'");
	$res_row = com_db_fetch_array($res_result);
...
}elseif($action=='DublicatEmail' || $action == 'BlankField' || $action == 'BannedEmail'){
...
<!-- ClickTale end of Top part -->
	...
	<div id="header">
		<!-- Shell -->
		<div class="shell">
			 <h1 id="logo"><a class="notext" href="<?=HTTP_SERVER?>index.php">CISOs on the Move</a></h1>
	    </div>
	</div>
	<!-- end Header -->
	...
	<div id="main">
	  ...
	  <div class="active-heading">Step 1<br />Provide Contact Information</div>
	  </div>
	  
	    <div class="step2">
	    <div class="step-box-heading">Step 2<br />Chose Subscription Level</div>
	    </div>
		  <div class="step3">
		   <div class="step-box-heading">Step 3<br />
	        Process Subscription Details</div>
		  </div>
				</div>
			<!-- step box -->
			<!-- field box -->	
			<div class="step-field-box">
					<form name="frm_pci" id="frm_pci" method="post" action="res-process.php?action=PCInformation<?PHP if($action=='back'){echo '&type=edit&resID='.$reg_id;}else{echo '&type=add';}?>" onsubmit="return pviValidationNew();">
