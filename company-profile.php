...
<body onLoad="initialize();" onfocus="doPopup();">
			...
			<div class="shell clearfix">
				<div class="social-media clearfix">
                	<?PHP if($companyRow['linkedin_link'] !='' || $companyRow['googleplush_link'] !='' || $companyRow['twitter_link'] !='' || $companyRow['facebook_link'] !=''){
						...
						?>
					<p>Social Media:</p>
                    <?PHP if($companyRow['linkedin_link'] !=''){ ?><a href="<?=$linkedin_url;?>" class="linkedin-icon" rel="nofollow">linkedin</a><?PHP } ?>
					<?PHP if($companyRow['googleplush_link'] !=''){ ?><a href="<?=$googleplush_url;?>" class="google-icon" rel="nofollow">google</a><?PHP } ?>
					<?PHP if($companyRow['twitter_link'] !=''){ ?><a href="<?=$twitter_url?>" class="twitter-icon" rel="nofollow">twitter</a><?PHP } ?>
					<?PHP if($companyRow['facebook_link'] !=''){ ?><a href="<?=$facebook_url?>" class="facebook-icon" rel="nofollow">facebook</a><?PHP } ?>
                    <?PHP } ?>
				</div>
				<!-- /social-media -->
		...
		<div class="profile-cnt">
			<div class="shell clearfix">
				<div class="profile-description">
                	<?PHP if($companyRow['company_logo'] !='' && $_SESSION['sess_is_user'] == 1) { ?>
					...
					<!-- /profile-img -->
					<div class="cl">&nbsp;</div>
					<?PHP		$about_company= $companyRow['about_company'];
	...
	<div class="company-section table-section">
		<div class="shell">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <?PHP while($perExeRow = com_db_fetch_array($perExeResult)){?>
				<tr>
	...
	<div class="company-section jobs-section">
		<div class="shell">
			...
			<?PHP 
				...
				?>
            		 <?PHP $job_desc =explode(" ", $jobRow['description']);
						$job_description='';
					...
					?>
                    <div class="job">
                        <h4><a href="javascript:ShowJobDetails('job_details_<?php echo $i?>');"><?=com_db_output($jobRow['job_title']);?><?PHP if($tot_day_diff<=30){echo '<span class="new-ico"></span>';}?></a></h4>
	...
	$fundingQuery = "SELECT funding_id,funding_amount,company_name,company_logo,cm.company_id,funding_date,funding_source from
					...
					".TABLE_COMPANY_MASTER." as cm
					where cm.company_id = cf.company_id and cm.company_id  = $company_id";
	...
	if($funding_num_row > 0)
	{
		...
		{
			$company_name = $fundingRow['company_name'];
		...
		?>
					...
					<div class="job">
					<?=$company_name?> raised <?=$funding_amount?> on <?=$funding_date?><br> 
					</div>
		...
		<div class="shell">
			...
			<?PHP
				$compQuery ="select company_id, company_name, company_website, company_logo, about_company from ".TABLE_COMPANY_MASTER." where company_id !='".$company_id."' and industry_id='".$companyRow['industry_id']."' ORDER BY RAND() LIMIT 5";
                                //$compQuery ="select company_id, company_name, company_website, company_logo, about_company from ".TABLE_COMPANY_MASTER." where company_id !='".$company_id."' and industry_id='".$companyRow['industry_id']."' and company_id in (543873,26327) LIMIT 5";
				...
				if($comp_num_rows>0){
                    ...
                    <div class="company">
                    	<?PHP if($compRow['company_logo'] !='' && $_SESSION['sess_is_user'] == 1) { ?>
