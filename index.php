<?php
//header('Location: https://www.execfile.com');
...
<head>
	...
	<title><?PHP if($current_page!='vigilant-appoints.php'){echo 'CISOsOnTheMove.com ::';}?> <?=$PageTitle;?></title>
...
<div id="main">
	<div class="shell">
		<div id="top-content">
			<div class="banner">
			<?PHP	$total_movement = com_db_GetValue("select count(move_id) from ".TABLE_MOVEMENT_MASTER." where status=0");
			...
			?>
				<div class="number">
					<span><? //=$total_30day_movement?>237</span>				
				</div>
				<div class="cnt">
					<h3>Senior CISOs who changed jobs in the last 30 days – out of  <span class="darkbg"><?=number_format($total_movement,0,0,",")?></span>  IT Security Execs in our database.</h3>
			...
			<div class="pic-slider">
				<ul>
				<?PHP  
                                //echo "<br>Query: select caption,image_path from ".TABLE_SLIDER_SHOW." where status=0";
                                $slide_result = com_db_query("select caption,image_path from ".TABLE_SLIDER_SHOW." where status=0");
					
                                //echo "<br>Count: ".com_db_num_rows($slide_result);
                                $res_count = com_db_num_rows($slide_result);
                                //echo "<pre>slide_result: ";   print_r($slide_result);   echo "</pre>";
                                
                               // while($slideRow = com_db_fetch_array($slide_result)){
                                //for($s=0;$s<=$res_count;$s++)
                                while($slideRow = mysqli_fetch_array($slide_result))
                                {
                                
                                    //$slideRow = com_db_fetch_array($slide_result[$s]);
                                            //echo "<br>Within";
                                            //echo "<pre>slideRow: ";   print_r($slideRow);   echo "</pre>";
					?>
		...
		<div id="content">
                    <div class="content">
                        <h3>IT Security Execs Recent Appointments and Promotions:</h3>
                        <div class="listing">
                            <div id="DivContactListing" class="table" >
                                ...
                                //echo "<br>select m.title,m.movement_url,m.personal_id,m.announce_date,p.first_name,p.last_name,m.company_id from ".TABLE_MOVEMENT_MASTER." as m, ".TABLE_PERSONAL_MASTER." as p where m.personal_id=p.personal_id and m.status=0 and m.announce_date>='".$last30Day."' and p.ciso_user = 1 order by announce_date desc<br>";
                                $movementResult = com_db_query("select m.title,m.movement_url,m.personal_id,m.announce_date,p.first_name,p.last_name,m.company_id from ".TABLE_MOVEMENT_MASTER." as m, ".TABLE_PERSONAL_MASTER." as p where m.personal_id=p.personal_id and m.status=0 and m.announce_date>='".$last30Day."' and p.ciso_user = 1 order by announce_date desc");
                                if($movementResult)
                                {
                                ...
                                while($moveRow = com_db_fetch_array($movementResult)){
                                        ...
                                        ?>
                                            ...
                                            <div id="DivCnt_<?=$ind?>" class="main_content" style="display:none;">
                                                ...
                                                <div class="dateleft3"><a href="<?=HTTP_SERVER?><?PHP if($_SESSION['sess_is_user'] == 1){ echo $moveRow['movement_url'];}else{echo 'provide-contact-information.php';}?>">Get Contact Details</a></div>
                                ...
                                <strong></strong>
                                                            ...
                                                            <div class="dateleft2"><?=$headline;?></div>
                                                            <div class="dateleft3"><a href="<?=HTTP_SERVER?><?PHP if($_SESSION['sess_is_user'] == 1){ echo $perRow['movement_url'];}else{echo 'provide-contact-information.php';}?>">Get Contact Details</a></div>
                                                            <div style="clear:both;"></div>
                                                    </div>
                                                    <?PHP
                                            } 
						...
						<div class="row">
							<div class="section">
								...
								<div class="check-section">
									...
									//echo "<br>tot_ind_num: ".$tot_ind_num;
			...
			<div class="cl">&nbsp;</div>
			<div class="partners">
				<h2>CISOsOnTheMove Provides Real Time Sales Leads to the Best Technology Companies: </h2>
				<div class="wrap">
					<?PHP
			...
			<div class="col">
				...
				<ul class="service">
					<li>
						...
						<p>Search, browse and download CISO profiles with their contact details in real-time. 14,000 profiles of decision makers at your finger tips.</p>
					</li>
					<li>
						<img src="css_new/images/i2.png" class="icon" alt="" />
						<h4>Timely Updates</h4>
						<p>Get real-time updates when CISOs get appointed or promoted, with updated contact details.</p>
					</li>
					<li>
			...
			<div class="col">
				...
				<div class="testimonials">
					<div class="item">
						<div class="entry">
							<div class="entry-i">
								<p>“CISOsOnTheMove has helped us to quickly take advantage of opportunities with key IT leaders, creating real client engagements and extraordinary return on investment."</p>
							</div>
						</div>
						<div class="author">
			...
			<div class="col">
				...
				<ul class="service type2">
					<li>
						...
						<h4>IT Security Executive Directory</h4>
						<p>
						<?PHP
						for($i=65; $i<=90; $i++){
...
<div id="footer">
	<div class="shell">
		...
		<p class="copy">© <?=date("Y");?> CISOsOnTheMove. All rights reserved.&nbsp;&nbsp;<a href="<?=HTTP_SERVER?>terms-use.html" style="text-decoration:underline; color:#000;">Terms of Service</a></p>
	</div>	
</div>
<!-- End of Footer -->
