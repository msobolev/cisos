...
//echo "<br>FAR: ".$current_funding_person = getSiteUsers('www.faraztestc.com');
...
function get_converted_status($pattern_for_type,$company_name = '',$first_name='',$last_name='',$source='',$amount='',$title = '')
{
    //echo "<br><br>Within function entered_pattern: ".$entered_pattern;
    //echo "<br>Funding amount within func: ".$amount;
    ...
    if(strpos($status_pattern,"*.") > -1)
        $status_pattern = str_replace("*.","* .",$status_pattern);
    ...
    foreach($appointment_pattern_arr as $index => $value)
    {
        ...
        if(strpos($value,"*") > -1)
        {
            ...
            //echo "<br>company_name: ".$company_name;
            
            if($exact_db_val == 'first_name')
            {
                $converted_status .= trim($first_name)." ";
            }
            elseif($exact_db_val == 'last_name')
            {
                $converted_status .= trim($last_name)." ";
            }
            ...
            elseif($exact_db_val == 'company')
            {
                $converted_status .= $company_name." ";
                 //echo "<br>Converted status in condition: ".$converted_status;    
            }
            ...
            {
                $converted_status .= $source." ";
            }
            elseif($exact_db_val == 'title')
            {
                $converted_status .= $title." ";
            }
            elseif($exact_db_val == 'amount')
            {
                //echo "<br>within AMOUNT text conversion to value: ".$amount;
                $converted_status .= $amount." ";
            }
        ...
        if(strpos($converted_status," .") > -1)
        $converted_status = str_replace(" .",".",$converted_status);
    ...
    //echo "<br>pattern_for_type: ".$pattern_for_type;
    if($pattern_for_type == 'fundings')
    {
        //echo "<br>first_name: ".$first_name;
        //echo "<br>last_name: ".$last_name;
        if($first_name == '' && $last_name == '')
        {
            $congrats_pos = strpos($converted_status,'.Congrats');
            //echo "<br>congrats_pos: ".$congrats_pos;
            if($congrats_pos > -1)
            {
                $converted_status = substr($converted_status,0,$congrats_pos);
            }
            $converted_status .= " ".$source;
            //$converted_status 
            //echo "<br>converted_status: ".$converted_status;
        }
        
        $converted_status = str_replace(".Congrats",". Congrats",$converted_status);
...
if($action == 'SearchResult' || $action == 'Directory'){
	$company_name		= com_db_input($_REQUEST['search_company_name']);
	$company_url		= $_REQUEST['search_company_url'];
	$industry 			= $_REQUEST['search_industry'];
	$state				= $_REQUEST['search_state'];
	$country			= $_REQUEST['search_country'];
	$from_date			= $_REQUEST['from_date'];
	$to_date 			= $_REQUEST['to_date'];
	$company_revenue 	= $_REQUEST['search_company_revenue'];
	$company_employee	= $_REQUEST['search_company_employee'];
    ...
    switch ($action) {
	  ...
	  case 'edit':
			...
			$demo_email_funding = com_db_output($data_edit['demo_funding']);
                        ...
                        $job_site = com_db_output($data_edit['job_site']);
			...
			//echo "<br>websiteFundingQuery: ".$websiteFundingQuery;
			...
			while($wfRow = com_db_fetch_array($wfResult)){
                        ...
                        // To get current funding person
                        //echo "<pre>wfArray:";   print_r($wfArray); echo "</pre>";
                        //echo "<br>company_website:".$company_website;
                        if(in_array('HR',$wfArray))
                        {
                            $current_funding_person = getSiteUsers($company_website);
                        }
                        
                        //echo "<br>current_funding_person: ".$current_funding_person;
                        
                        // getting  HR Executives for this company
                        $ctoExecutives = getExecutives('cto',$company_id);
                        $cfoExecutives = getExecutives('cfo',$company_id);
                        $hrExecutives  = getExecutives('hr',$company_id);
                        $cloExecutives  = getExecutives('clo',$company_id);
                        $cmoExecutives  = getExecutives('cmo',$company_id);
                        $cisoExecutives  = getExecutives('ciso',$company_id);
                       // echo "<pre>hrExecutives:";   print_r($hrExecutives);   echo "</pre>";
                        
                        com_db_connect() or die('Unable to connect to database server!');
                        //echo "CTOEXECUTIVES : ".$ctoExecutives; 
		...
		break;	
            ...
            case 'editsave':
                ...
                $funding_person_names = array();
                $ctositenameFunding = array();
                $person_details = "";
                $hr_funding_person_id = $_POST['hre_funding_exe'];
                if($hr_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'HR';
                    $site_to_set = 'hr';

                    //echo "<br>hr_funding_person_id: ".$hr_funding_person_id;
                    $person_details = set_funding_person($site_to_set,$hr_funding_person_id,$cID);

                    //echo "<pre>person_details: ";   print_r($person_details);   echo "</pre>"; 

                    if($hr_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);


                    //echo "<pre>funding_person_names after HR: ";   print_r($funding_person_names);   echo "</pre>"; 

                }
                echo "<pre>funding_person_names after HR: ";   print_r($funding_person_names);   echo "</pre>"; 

                $cfo_funding_person_id = $_POST['cfo_funding_exe'];
                if($cfo_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'CFO';
                    $site_to_set = 'cfo';
                    $person_details = set_funding_person($site_to_set,$cfo_funding_person_id,$cID);
                    if($cfo_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);
                }


                $clo_funding_person_id = $_POST['clo_funding_exe'];
                if($clo_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'CLO';
                    $site_to_set = 'clo';
                    $person_details = set_funding_person($site_to_set,$clo_funding_person_id,$cID);
                    if($clo_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);
                }

                $cmo_funding_person_id = $_POST['cmo_funding_exe'];
                if($cmo_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'CMO';
                    $site_to_set = 'cmo';
                    $person_details = set_funding_person($site_to_set,$cmo_funding_person_id,$cID);
                    if($cmo_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);
                }

                $cto_funding_person_id = $_POST['cto_funding_exe'];
                if($cto_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'CTO';
                    $site_to_set = 'cto';
                    $person_details = set_funding_person($site_to_set,$cto_funding_person_id,$cID);
                    if($cto_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);
                }
                
                
                $ciso_funding_person_id = $_POST['ciso_funding_exe'];
                if($ciso_funding_person_id != '')
                {
                    $ctositenameFunding[] = 'CTO';
                    $site_to_set = 'cto';
                    $person_details = set_funding_person($site_to_set,$ciso_funding_person_id,$cID);
                    if($cto_funding_person_id != 'Select Funding Person')
                        $funding_person_names = explode(":", $person_details);
                }
                ...
                //echo "<pre>funding_person_names at end: ";   print_r($funding_person_names);   echo "</pre>"; 
                ...
                $company_name = trim(com_db_input($_POST['company_name']));
                ...
                $address = trim(com_db_input($_POST['address']));
                $address2 = trim(com_db_input($_POST['address2']));
                $city = trim(com_db_input($_POST['city']));
                $state = com_db_input($_POST['state']);
                $country = com_db_input($_POST['country']);
                $zip_code = trim(com_db_input($_POST['zip_code']));
                $phone = com_db_input($_POST['phone']);
                $fax = com_db_input($_POST['fax']);

                $about_company = mysql_real_escape_string(strip_tags($_POST['about_company']));
                ...
                if(isset($_POST['secondary_websites']))
                {
                    ...
                    $company_urls = trim($company_urls,",");
                    if($company_urls != '')
                    {
                        $ins_urls_query = "UPDATE " . TABLE_COMPANY_MASTER . " SET company_urls = '" . $company_urls . "' WHERE company_id = '" . $cID ."'";
                        com_db_query($ins_urls_query);
                    }  
                ...
                leadership_page ='".$leadership_page."',email_pattern_id = '".$email_pattern."', address = '" . $address ."', address2 = '".$address2."', city = '".$city."', state = '".$state."', country = '".$country."', zip_code = '".$zip_code."', phone = '".$phone."',fax = '".$fax."',company_urls = '".$company_urls."',
                about_company = '".$about_company."', facebook_link = '".$facebook_link."', linkedin_link = '".$linkedin_link."', twitter_link = '".$twitter_link."', googleplush_link = '".$googleplush_link."',demo_job='".$demo_job."', modify_date = '".$modify_date."',create_by='".$create_by."',admin_id='".$login_id."',job_site='".$job_site."',demo_funding='".$demo_email_funding."'$email_domain_update where company_id = '" . $cID . "'";
                //echo "<br>query: ".$query;
                ...
                for($j=0; $j<sizeof($job_title_arr);$j++)
                {
                    ...
                    //Funding insert
                            ...
                            echo "<br>size of ctositenameFunding array:".$ctositenameFunding;
                            echo "<br>size of added_funding_ids array:".$added_funding_ids;
                            echo "<pre>ctositenameFunding: ";   print_r($ctositenameFunding);   echo "</pre>";
                            ...
                            for($cf=0; sizeof($ctositenameFunding) > $cf ;$cf++){
                                for($af=0; sizeof($added_funding_ids) > $af ;$af++){
                                    echo "<br>Inserti Q: insert into ".TABLE_COMPANY_FUNDING_WEBSITE." (company_id,funding_id,website) values('".$cID."','".$added_funding_ids[$af]."','".$ctositenameFunding[$cf]."')";
                            ...
                            if($job_added == 1 || $funding_added == 1)
                            {    
                                //$_SESSION['text']   = rawurlencode($status_pattern); //'forbuff56';
                                $_SESSION['p']      = $p;
                                $_SESSION['type']   = "";   
                                //echo "<br>session text: ".$_SESSION['text'];
                                ...
                                $status_pattern = "TestfundingBufferStatus";
                                ...
                                if($job_added == 1)
                                {    
                                //    $status_pattern = get_converted_status("jobs",$first_name,$last_name,$title,$company_name_full,$movement_type,$more_link);
                                    $status_pattern = get_converted_status("jobs",$company_name,$first_name,$last_name,$source,$amount,$job_title);
                                }
                                */

                                if($funding_added == 1)
                                {   
                                    //echo "<br>Funding amount before func call: ".$funding_amount;
                                    //    $status_pattern = get_converted_status("jobs",$first_name,$last_name,$title,$company_name_full,$movement_type,$more_link);
                                    $status_pattern = get_converted_status("fundings",$company_name,$funding_person_names[0],$funding_person_names[1],$funding_source,$funding_amount);
                                    $text = rawurlencode($status_pattern);

                                    $params = 'text='.$text.'&profile_ids[]='.HREXECS_TWITTER_PROFILE_ID; // hrexecs
                                ...
                                //echo "<br>status_pattern: ".$status_pattern; die();
                            }  
	...
	case 'addsave':
			...
			if($isCompanyPresent ==''){
				...
				if(isset($_POST['secondary_websites']))
				{
                                    ...
                                    if($company_urls != '')
                                    {
                                        $ins_urls_query = "UPDATE " . TABLE_COMPANY_MASTER . " SET company_urls = '" . $company_urls . "' WHERE company_id = '" . $insert_id ."'";
                                        com_db_query($ins_urls_query);
                                    }    
				}
				...
				(company_id,company_name, company_website, company_revenue, company_employee, company_industry, ind_group_id, industry_id,leadership_page,email_pattern_id, address, address2, city, state, country, zip_code, phone, fax, about_company, facebook_link ,linkedin_link ,twitter_link, googleplush_link,demo_job, add_date, create_by,admin_id, status,demo_funding,job_site,company_urls) 
				...
				for($j=0; $j<sizeof($funding_date_arr);$j++)
                                ...
                                {    
                                    ...
                                    {   
                                        //echo "<br>Funding amount before func call: ".$funding_amount;
                                        //    $status_pattern = get_converted_status("jobs",$first_name,$last_name,$title,$company_name_full,$movement_type,$more_link);
                                        $status_pattern = get_converted_status("fundings",$company_name,$first_name,$last_name,$funding_source,$funding_amount);
                                    }
                                    */
                                    //echo "<br>status_pattern: ".$status_pattern; die();
...
function show_funding_personal()
{ 
    //var f_sb = document.getElementById("ctositenameFunding");
    //var f_sb_val = f_sb.options[f_sb.selectedIndex].value;
    //var e = document.getElementById("ctositenameFunding");
    //typeof document.getElementsByName("+field+")[0] != 'undefined'
    //alert("within if:"+typeof e.options[e.selectedIndex]);    
    
    var x = document.getElementById("ctositenameFunding");
    var hr_selected = 0;
    var selsite = '';
    for (var i = 0; i < x.options.length; i++) 
    {
        if(x.options[i].selected)
        {
            ...
            var this_sel = x.options[i].value;
            if(this_sel == 'HR')
            {
                hr_selected = 1;
                selsite = "HR";
    ...
    if(typeof x.options[x.selectedIndex] != 'undefined')
    {   
        //var selsite = e.options[e.selectedIndex].value[1];
        var comp_url = document.getElementById("company_website").value;
        if(comp_url != '')
        {    
            if(hr_selected == 1)
            {
                ...
                var url="getuser.php";
                url=url+"?type=GetSiteUsrs&site="+selsite+"&comp_url="+comp_url;
...
function enableBtn(selectedVal)
{
    //alert("heer: "+selectedVal);
    if(selectedVal.indexOf("Select Funding Person")> -1)
    {
...
function check_selection()
{
 /*   
    var selected_person = ''; 
    var cfo_selected_person = ''; 
    var clo_selected_person = ''; 
    var cmo_selected_person = ''; 
    var hre_selected_person = ''; 
    
    if(document.getElementById("cto_funding_exe"))
    {    
        var dd = document.getElementById("cto_funding_exe");
        var selected_person = dd.options[dd.selectedIndex].value;
        
    }
    
    if(document.getElementById("cfo_funding_exe"))
    { 
        var cfo_fp = document.getElementById("cfo_funding_exe");
        var cfo_selected_person = cfo_fp.options[cfo_fp.selectedIndex].value;
        
    }
    
    if(document.getElementById("clo_funding_exe"))
    { 
        var clo_fp = document.getElementById("clo_funding_exe");
        var clo_selected_person = clo_fp.options[clo_fp.selectedIndex].value;
    }
    
    if(document.getElementById("cmo_funding_exe"))
    {
        var cmo_fp = document.getElementById("cmo_funding_exe");
        var cmo_selected_person = cmo_fp.options[cmo_fp.selectedIndex].value;
    }
    
    if(document.getElementById("hre_funding_exe"))
    {
        var hre_fp = document.getElementById("hre_funding_exe");
        var hre_selected_person = hre_fp.options[hre_fp.selectedIndex].value;
    }
    
    
    if(selected_person != 'Select Funding Person' && selected_person != '')
    {    
        document.getElementById("submitBtn").disabled = false; 
        document.getElementById("submitBtn").title = ""; 
    }
    else
    if(cfo_selected_person != 'Select Funding Person' && cfo_selected_person != '')
    {    
        document.getElementById("submitBtn").disabled = false; 
        document.getElementById("submitBtn").title = ""; 
    }
    else
    if(clo_selected_person != 'Select Funding Person' && clo_selected_person != '')
    {    
        document.getElementById("submitBtn").disabled = false; 
        document.getElementById("submitBtn").title = ""; 
    }
    else
    if(cmo_selected_person != 'Select Funding Person' && cmo_selected_person != '')
    {    
        document.getElementById("submitBtn").disabled = false; 
        document.getElementById("submitBtn").title = ""; 
    }
    else
    if(hre_selected_person != 'Select Funding Person' && hre_selected_person != '')
    {    
        document.getElementById("submitBtn").disabled = false; 
        document.getElementById("submitBtn").title = ""; 
    }
    ...
    {
        document.getElementById("submitBtn").disabled = true; 
        document.getElementById("submitBtn").title = "To save funding information please add one person from the drop down list to at least one site (HR, CTO, CFO, CMO or Legal)"; 
    }    
    */
    //alert("heree:"+selected_person);
}
    ...
    <tr>
      ...
      <td align="center" valign="top"><table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<?php echo number_pages($main_page, $p, $total_data, 8, $items_per_page,'&action=Directory&search_first_name='.$_REQUEST['search_first_name'].'&search_last_name='.$_REQUEST['search_last_name'].'&from_date='.$_REQUEST['from_date'].'&to_date='.$_REQUEST['to_date'].'&items_per_page='.$items_per_page);?>
			<br /><br />
	  		</td>
		</tr>
	
	<?PHP }elseif($action=='add' || $action ==''){ ?>
		<tr>
          <td align="center" valign="top">
		 ...
		 <!--start iner table  -->
		  ...
		  <table width="95%" align="left" cellpadding="5" cellspacing="5" border="0">
            ...
            <tr>
                <td align="left">
                    Select Job Site&nbsp;
                    <select name="job_site" id="job_site">
            ...
            <tr>
                <td align="right" style="color: #064962;font-family: Arial;font-size:13px;font-weight:bold;padding-right:150px;">
                    <input type="checkbox" name="demo_email_funding" id="demo_email_funding" value="1" <?PHP if ($demo_email_funding==1){echo 'checked="checked"';} ?> onclick="fundingTiggerOnOff();" /> Add to demo? <!--   -->
		</td>
            </tr>
            ...
            <tr>
                <td align="right" style="color: #064962;font-family: Arial;font-size:13px;font-weight:bold;padding-right:150px;">
              	<div id="divFundingWebsiteTigger" style="display:<?PHP if ($demo_job==1){echo 'block';}else{echo 'none';}?>;">
                    <div class="row">
                        <select onchange="show_funding_personal()" name="ctositenameFunding[]" id="ctositenameFunding" class="websitename" multiple data-placeholder="Please select website">
                            <option value="CTO">CTO</option>
                            <option value="CLO">CLO</option>
                            <option value="CMO">CMO</option>
                            <option value="CFO">CFO</option>
            ...
            <tr>            
                <td><div id="divFundingPersonal"></div>
                </td>
            </tr>    
		  ...
		  <input type="hidden" name="hidden_email_domain" id="hidden_email_domain" value="<?=$email_domain?>">
            ...
            <tr>
                <td align="left">
                    ...
                    <select name="job_site" id="job_site">
                        <option value="">Select Job site</option>
                        <option <?PHP if($job_site == 'cto')  echo "selected"; else echo ""; ?> value="cto">CTO</option>
                        <option <?PHP if($job_site == 'cfo')  echo "selected"; else echo ""; ?> value="cfo">CFO</option>
                        <option <?PHP if($job_site == 'clo')  echo "selected"; else echo ""; ?> value="clo">CLO</option>
                        <option <?PHP if($job_site == 'cmo')  echo "selected"; else echo ""; ?> value="cmo">CMO</option>
                        <option <?PHP if($job_site == 'hr')  echo "selected"; else echo ""; ?> value="hr">HR</option>
                    </select>
                </td>	
            </tr>              
            ...
            <tr>
             ...
             <tr>
			  <td align="right" style="color: #064962;font-family: Arial;font-size:13px;font-weight:bold;padding-right:150px;">
              	<input type="checkbox" name="demo_job" id="demo_job" value="1" <?PHP if ($demo_job==1){echo 'checked="checked"';} ?> onclick="JobTiggerOnOff();" /> Add to demo?
                &nbsp;&nbsp;
				<input type="checkbox" name="buyside_job" id="buyside_job" value="1" <? if ($buyside_job==1){echo 'checked="checked"';} ?>  onclick="ExecutiveJobTiggerOnOff();" /> Add to buyside demo
            ...
            <tr>
                <td align="right" style="color: #064962;font-family: Arial;font-size:13px;font-weight:bold;padding-right:150px;">
                    <div id="divWebsiteTigger" style="display:<? if ($demo_job==1){echo 'block';}else{echo 'none';}?>;">
                            <div class="row">
                            <select name="ctositename[]" id="ctositename" class="websitename" multiple data-placeholder="Please select website">
                                <option value="CTO" <?PHP if(in_array('CTO',$wsArray)){echo 'selected="selected"';} ?> >CTO</option>
                                <option value="CLO" <?PHP if(in_array('CLO',$wsArray)){echo 'selected="selected"';} ?>>CLO</option>
                                <option value="CMO" <?PHP if(in_array('CMO',$wsArray)){echo 'selected="selected"';} ?>>CMO</option>
                                <option value="CFO" <?PHP if(in_array('CFO',$wsArray)){echo 'selected="selected"';} ?>>CFO</option>
                                <option value="HR" <?PHP if(in_array('HR',$wsArray)){echo 'selected="selected"';} ?>>HR</option>
                            </select>
                        </div><!-- /.row -->
                    </div> 			
                    ...
                    <div id="divBuysideWebsiteTigger" style="display:<? if ($buyside_job==1){echo 'block';}else{echo 'none';}?>;">
                        <div class="row">
                            <select name="ctositenameBuyside[]" id="ctositenameBuyside" class="websitename" multiple data-placeholder="Please select website">
                                <option value="CTO" <?PHP if(in_array('CTO',$bsArray)){echo 'selected="selected"';} ?> >CTO Buyside</option>
                                <option value="CLO" <?PHP if(in_array('CLO',$bsArray)){echo 'selected="selected"';} ?> >CLO Buyside</option>
                                <option value="CMO" <?PHP if(in_array('CMO',$bsArray)){echo 'selected="selected"';} ?> >CMO Buyside</option>
                                <option value="CFO" <?PHP if(in_array('CFO',$bsArray)){echo 'selected="selected"';} ?> >CFO Buyside</option>
                                <option value="HR" <?PHP if(in_array('HR',$bsArray)){echo 'selected="selected"';} ?> >HR Buyside</option>
                            </select>
                        </div><!-- /.row -->
                    </div>  
                </td>	
			...
			<!-- onclick="fundingTiggerOnOff();" -->
                        <tr>
                            <td align="right" style="color: #064962;font-family: Arial;font-size:13px;font-weight:bold;padding-right:150px;">
                               <input type="checkbox" name="demo_email_funding" id="demo_email_funding" value="1" <?PHP if ($demo_email_funding==1){echo 'checked="checked"';} ?>   /> Add to demo?
                            </td>
                        </tr>
            ...
            <tr>            
                <td style="text-align:right; padding-right:150px;]">
                    <div id="divFundingPersonal">
                    <?PHP 
                    if($current_funding_person != '')
                    {    
                        echo $current_funding_person;
                    }
                    ?>
                    </div>
                </td>
            ...
            //$cto_sel_pos = strpos($ctoExecutives," selected");
            //$hr_sel_pos = strpos($hrExecutives," selected");
            //if(($ctoExecutives != '' && $cto_sel_pos == '') && ($hrExecutives != '' && $hr_sel_pos == ''))
            
            $btn_status = "";
            $btn_title = "";
            $cto_sel_pos = strpos($ctoExecutives," selected");
            $cfo_sel_pos = strpos($cfoExecutives," selected");
            $clo_sel_pos = strpos($cloExecutives," selected");
            $cmo_sel_pos = strpos($cmoExecutives," selected");
            $hr_sel_pos = strpos($hrExecutives," selected");
            $ciso_sel_pos = strpos($cisoExecutives," selected");
            
            $have_funding = 0;
            if($funding_num_row > 0)
                $have_funding = 1;
            //if($cto_sel_pos == '' && $cfo_sel_pos == '' && $clo_sel_pos == '' && $cmo_sel_pos == '' && $hr_sel_pos == '' && $demo_email_funding == 1)
            if($cto_sel_pos == '' && $cfo_sel_pos == '' && $clo_sel_pos == '' && $cmo_sel_pos == '' && $hr_sel_pos == '' && $have_funding == 1)
            {
                $btn_status = "disabled";
                $btn_title = "To save funding information please add one person from the drop down list to at least one site (HR, CTO, CFO, CMO or Legal)";
            }
            ...
            if($hrExecutives != '')
            //if($hrExecutives != '' && in_array('HR',$wfArray)) // Added this line on 22nd july 2016
            {                                                
            ?>
            <tr>
                <td style="">
                <?PHP echo $hrExecutives;  ?>
                </td>
            </tr>                  
            <?PHP
            }
            ...
            <tr>
            	<td align="center">
                    <table width="50%" border="0" cellpadding="4" cellspacing="4">
                        <tr>
                            <td width="22%">
                                <!--<input type="submit" value="Update Company" class="submitButton" id="submitBtn" <?=$btn_status?> title="<?=$btn_title?>" />-->
                                <input type="submit" value="Update Company" class="submitButton"  id="submitBtn"   title="<?=$btn_title?>" />  <!-- <? //=$btn_status?> --> <!-- FA UNDO -->
                            </td>
