...
if($action == 'SearchResult'){
	...
	$search_qry='';
	if($personal_id!=''){
		$search_qry .= " pp.personal_id = '".$personal_id."'";
	}
	if($job_title!=''){
		if($search_qry==''){
			$search_qry .= " pp.title like '".$title."%'";
		}else{
			$search_qry .= " and pp.title like '".$title."%'";
		}	
	}
	...
	if($search_qry==''){
		...
		$sql_query = "select pm.first_name,pm.last_name,pp.* from " . TABLE_PERSONAL_MASTER . " pm, ".TABLE_PERSONAL_PUBLICATION." pp where (pm.personal_id=pp.personal_id) and ".$search_qry." order by publication_id desc";
	}
...
$pID = (isset($_GET['pID']) ? $_GET['pID'] : $select_data[0]);
    ...
    switch ($action) {
	  ...
	  case 'delete':
			...
			com_db_query("delete from " . TABLE_PERSONAL_PUBLICATION . " where publication_id = '" . $pID . "'");
		 	com_redirect("publication.php?p=" . $p . "&msg=" . msg_encode("Personal publication deleted successfully"));
		
		break;	
	  case 'edit':
     		
	   		$query_edit=com_db_query("select * from " . TABLE_PERSONAL_PUBLICATION . " where publication_id = '" . $pID . "'");
			...
			$pdt = explode('-',$data_edit['publication_date']);
			$publication_date = 	$pdt[1].'/'.$pdt[2].'/'.$pdt[0];
			$link = com_db_output($data_edit['link']);
		...
		case 'editsave':
			...
			$pdt = explode('/',$_POST['publication_date']);//mmddyyyy
			$publication_date = $pdt[2].'-'.$pdt[0].'-'.$pdt[1];
			$link = com_db_input($_POST['link']);
			
			$query = "update " . TABLE_PERSONAL_PUBLICATION . " set personal_id = '" . $personal_id ."', title = '".$title."',publication_date='".$publication_date."',link='".$link."' where publication_id = '" . $pID . "'";
			
			com_db_query($query);
	  		com_redirect("publication.php?p=". $p ."&pID=" . $pID . "&msg=" . msg_encode("Personal publication update successfully"));
	...
	case 'addsave':
			...
			values ('$personal_id', '$title','$publication_date','$link','$add_date','$status')";
			com_db_query($query);
	  		com_redirect("publication.php?p=" . $p . "&msg=" . msg_encode("Personal New publication added successfully"));
		 
		break;	
		
	case 'detailes':
			$query_edit ="select pm.first_name,pm.last_name,pp.* from " 
							.TABLE_PERSONAL_MASTER. " as pm, " 
							.TABLE_PERSONAL_PUBLICATION." as pp														
							where (pm.personal_id=pp.personal_id) and pp.publication_id = '" . $pID . "'";
...
function BoardsSearch(){
	window.location ='publication.php?action=BoardsSearch';
}
function confirm_del(nid,p){
	var agree=confirm("Publication will be deleted. \n Do you want to continue?");
	if (agree)
		window.location = "publication.php?pID=" + nid + "&p=" + p + "&action=delete";
	else
		window.location = "publication.php?pID=" + nid + "&p=" + p ;
}
...
<div id="light" class="white_content" style="display:<? if($action=='BoardsSearch'){ echo 'block';} else { echo 'none'; } ?>; height:auto;">
		...
		<form name="frmSearch" id="frmSearch" method="post" action="publication.php?action=SearchResult">
		 ...
		 <!-- <tr>
		  ...
		  <tr>
			<td align="center" valign="top">&nbsp;
		  	<td align="left" valign="top"><input type="submit" value="Search" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="Cancel" onclick="window.location='publication.php'" /></td>
		  </tr>
		  <tr>
			<td align="center" valign="top">&nbsp;</td>
			<td align="center" valign="top">&nbsp;</td>
                ...
                <table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td align="center" valign="top"><table width="928" border="0" align="center" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="196" align="left" valign="top"><a href="publication.php"><img src="<?=HTTP_SERVER.DIR_WS_HTTP_FOLDER.DIR_IMAGES?>logo.jpg"  alt="Logo" width="196" height="32" border="0" title="Logo" /></a></td>
                    ...
                    <td align="right" valign="top">
                        <table width="95%" height="33" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                              ...
                              <td align="left" valign="middle"><a href="#"><img src="images/folder-icon.jpg" border="0" width="22" height="22" alt="Directory" title="Directory" onclick="window.location='publication.php?action=Directory'"  /></a></td>
                              <td align="left" valign="middle" class="nav-text">Directory</td>
                              <td width="3%" align="right" valign="middle"><a href="#"><img src="<?=HTTP_SERVER.DIR_WS_HTTP_FOLDER.DIR_IMAGES?>add-icon.jpg" border="0" width="25" height="28" alt="Add Contact" title="Add Contact" onclick="window.location='publication.php?action=add'"  /></a></td>
          ...
          <td align="center" valign="top"><table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td align="left" valign="middle" class="advance-search-page-heading-bg"><table width="960" border="0" align="left" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td align="left" valign="middle" class="press-release-page-title-text">Personal Publication : </td>
                  </tr>
                  <tr>
...
<? if($action=='Directory' || $action =='save' || $action=='SearchResult'){	?>	
        <tr>
		  ...
		  <form name="topicform" id="topicform" method="post" action="publication.php?action=alldelete&p=<?=$p?>">
			  ...
			  <tr>
				...
				<td width="315" height="30" align="center" valign="middle" class="right-border"><span class="right-box-title-text">Publication Title</span> </td>
			...
			?>          
			  <tr>
				...
				<td height="30" align="left" valign="middle" class="right-border-text">&nbsp;<a href="publication.php?action=detailes&p=<?=$p?>&pID=<?=$data_sql['publication_id'];?>"><?=com_db_output($data_sql['first_name'].' '.$data_sql['last_name'])?></a></td>
				...
				<td height="30" align="center" valign="middle" class="left-border">
					<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
					  <tr> 
					   	<td width="50%" align="center" valign="middle" class="actionText"><a href="#"><img src="images/small-edit-icon.gif" width="16" height="16" alt="Edit" title="Edit" border="0" onclick="window.location='publication.php?&p=<?=$p;?>&pID=<?=$data_sql['publication_id'];?>&action=edit'" /></a><br />
						  Edit</td>
						<td width="50%" align="center" valign="middle" class="actionText"><a href="#"><img src="images/small-delet-icon.gif" width="16" height="16" alt="Delete" title="Delete" border="0" onclick="confirm_del('<?=$data_sql['publication_id'];?>','<?=$p;?>')" /></a><br />
						  Delete</td>
					  </tr>
					</table>
               </td>
	...
	<? }elseif($action=='add' || $action ==''){ ?>
		<tr>
          ...
          <tr>
              ...
              <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="left" valign="middle" class="heading-text">Publication Manager :: Add Personal Publication</td>
                </tr>
              </table></td>
          </tr>
          <tr>
		 ...
		 <!--start iner table  -->
		  <form name="frmDataAdd" id="frmDataAdd" method="post" action="publication.php?action=addsave" onsubmit="return  chk_form_Add();">
            ...
            <tr>
			  <td align="left">
					...
					<tr>
                      ...
                      <td width="75%" align="left" class="page-text" valign="top">
                    ...
                    <tr>
                      <td width="25%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;Publication Title:</td>
                    ...
                    <tr>
                      ...
                      <td width="75%" align="left" valign="top"><input type="text" name="publication_date" id="publication_date" size="30" value="<?=$publication_date?>" />&nbsp;<a href="javascript:NewCssCal('publication_date','mmddyyyy','arrow')"><img src="images/calender-icon.gif" alt="" width="22" height="14" border="0" title="" /></a></td>	
                    </tr>
                    <tr>
                      <td width="25%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;Publication url:</td>
                      <td width="75%" align="left" valign="top">
                       	<input type="text" name="link" id="link" size="30" value="" />
                      </td>	
                    </tr>
                  </table>
              </td>	
            ...
            <tr>
            	<td align="center">
                	<table width="50%" border="0" cellpadding="4" cellspacing="4">
                    	<tr><td><input type="submit" value="Add Publication" class="submitButton" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="submitButton" value="Cancel" onclick="window.location='publication.php?p=<?=$p;?>'" /></td></tr>
                    </table>
                </td>
             </tr>
	...
	<? }elseif($action=='edit'){ ?>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
              ...
              <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="left" valign="middle" class="heading-text">Publication Manager :: Edit Personal Publication </td>
                </tr>
              </table></td>
          </tr>
          <tr>
		 ...
		 <!--start iner table  -->
		<form name="frmDataEdit" id="frmDataEdit" method="post" action="publication.php?action=editsave&pID=<?=$pID;?>&p=<?=$p;?>" onsubmit="return  chk_form();">
		  <table width="100%" align="center" cellpadding="5" cellspacing="5" border="0">
            ...
            <tr>
			  <td align="left">
                    ...
                    <tr>
                      <td width="25%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;Publication url:</td>
                      <td width="75%" align="left" valign="top">
                       	<input type="text" name="link" id="link" size="30" value="<?=$link?>" />
                      </td>	
                    </tr>
            ...
            <tr>
            	<td align="center">
                	<table width="50%" border="0" cellpadding="4" cellspacing="4">
                    	<tr>
                        	<td width="17%"><input type="submit" value="Update Publication" class="submitButton" /></td>
                            <td width="83%"><input type="button" class="submitButton" value="Cancel" onclick="window.location='publication.php?p=<?=$p;?>'" /></td>
                        </tr>
                    </table>
                </td>
             </tr>
	...
	<? }elseif($action=='detailes'){?>
		 <tr>
          ...
          <tr>
              <td align="center" valign="middle" style="background-color:#CCC;height:30px;"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="left" valign="middle" class="heading-text">Publication Manager :: Personal Publication Details </td>
                </tr>
              </table></td>
          </tr>
          <tr>
		 ...
		 <!--start iner table  -->
         	...
         	<table width="100%" align="center" cellpadding="5" cellspacing="5" border="0">
                ...
                <tr>
                  <td width="25%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;Date:</td>
                  <td width="75%" align="left" valign="top" class="page-text"><?=$publication_date?></td>	
                </tr>
                <tr>
                  <td width="25%" align="left" class="page-text" valign="top">&nbsp;&nbsp;&nbsp;Publication url:</td>
		 ...
		 <tr>
            	<td align="center">
                	<table width="50%" border="0" cellpadding="4" cellspacing="4">
                    	<tr><td><input type="button" class="submitButton" value="Back" onclick="window.location='publication.php?p=<?=$p;?>'" /></td></tr>
                    </table>
                </td>
             </tr>
            </table></td>
