...
if(strpos($_SERVER['HTTP_REFERER'],"execfile.com/") > -1 )
{
    $from_exec = 1;
}
...
$personal_query = 'select mm.company_id,mm.title,mm.announce_date,pm.*,cm.company_name,ind_group_id,cm.industry_id,cm.company_website,cm.about_company from '					
					...
					where mm.personal_id=pm.personal_id and mm.company_id=cm.company_id and pm.personal_id = "'.$personal_id.'" and pm.status = 0 order by mm.announce_date desc';
		...
		<div class="profile-heading">
			<div class="shell clearfix">
				<div class="social-media clearfix">
                	<?PHP if($personal_row['linkedin_link'] !='' || $personal_row['googleplush_link'] !='' || $personal_row['twitter_link'] !='' || $personal_row['facebook_link'] !=''){
						...
						?>
					<p>Social Media:</p>
                    <?PHP if($personal_row['linkedin_link'] !=''){ ?><a href="<?=$linkedin_url?>" class="linkedin-icon" rel="nofollow">linkedin</a><?PHP } ?>
					<?PHP if($personal_row['googleplush_link'] !=''){ ?><a href="<?=$googleplush_url;?>" class="google-icon" rel="nofollow">google</a><?PHP } ?>
					<?PHP if($personal_row['twitter_link'] !=''){ ?><a href="<?=$twitter_url;?>" class="twitter-icon" rel="nofollow">twitter</a><?PHP } ?>
					<?PHP if($personal_row['facebook_link'] !=''){ ?><a href="<?=$facebook_url;?>" class="facebook-icon" rel="nofollow">facebook</a><?PHP } ?>
                    <?PHP } ?>
				</div>
				<!-- /social-media -->
		...
		<div class="profile-cnt">
			<div class="shell clearfix">
				...
				$ImageDisplay=trim($imageStatus);
   				?>
                  	<?PHP if($personal_row['personal_image']!='' &&  $ImageDisplay=='ON'){ ?>
                    ...
                    elseif($personal_row['personal_image'] !='' && $from_exec == 1 )
                    {
                    ?>
                    <div class="profile-img" style="background-color:#FFF;">
                        <img src="<?=HTTP_SERVER?>personal_photo/thumb/<?=$personal_row['personal_image']?>" alt="<?=com_db_output($personal_row['first_name'].' '.$personal_row['last_name']);?>" title="<?=com_db_output($personal_row['first_name'].' '.$personal_row['last_name'])?>" height="200px" width="200px" style="position:absolute;top:0;bottom:0;margin:auto;"/>
                    </div>
                    <?PHP                
                    }
				...
				<div class="profile-info">
					...
					<ul class='table-list'>
						...
						<?PHP if($_SESSION['sess_is_user'] != 1 && $from_exec == 0){ ?>
                        <li><strong>Email:</strong><?=$email_str;?></li>
						<li><strong>Phone:</strong><?=$phone_str?></li>
                        <?PHP }elseif($_SESSION['sess_is_user'] == 1 || $from_exec == 1){ ?>
	...
	<div class="interests">
		<div class="shell">
			...
			<ul>
				...
				<?PHP if($personal_row['edu_ugrad_degree'] !='' || $personal_row['edu_grad_degree']){ ?>
				<li>
					<div class="heading">
					...
					<div class="interest-cnt">
						<p>
                        <?PHP if($personal_row['edu_ugrad_degree'] !='' ){ 
							...
							while($awardsRow = com_db_fetch_array($awardsResult)){
								$awards_date = $awardsRow['awards_date'];
				 				$awards_dt = explode("-", $awards_date);
                                                                ...
                                                                $awards_complete_dt = $awards_dt[1]."/".$awards_dt[2]."/".$awards_dt[0];	
                                                                
                                                                $check_award = substr($awardsRow['awards_title'],strlen($awardsRow['awards_title'])-5,5);
                                                                
                                                                if($check_award == 'Award' || $check_award == 'award')
								{
									$award_text = "";
								}
								else
								...
								if($awards==1){
									//echo $personal_row['first_name'].' was selected as the '.$awardsRow['awards_title'].' by the '.$awardsRow['awards_given_by'].' in '.$awards_dt[0].'.';
                                                                        echo $personal_row['first_name'].' received '.$awardsRow['awards_title'].$award_text.' from '.$awardsRow['awards_given_by'].' on '.$awards_complete_dt.'.';
								}else{
									//echo ' '.$personal_row['first_name'].' was selected as the '.$awardsRow['awards_title'].' by the '.$awardsRow['awards_given_by'].' in '.$awards_dt[0].'.';
                                                                        echo ' '.$personal_row['first_name'].' received '.$awardsRow['awards_title'].$award_text.' from '.$awardsRow['awards_given_by'].' on '.$awards_complete_dt.'.';
								}
	...
	<div class="list-profiles">
		<div class="shell clearfix">
			...
			<div class="more-profiles">
				<h4>
					Other IT Security Executives<br />
					at <?=com_db_output($personal_row['company_name'])?></em>:
				</h4>
				<?PHP while($exeRow = com_db_fetch_array($exeResult)){ ?>
				<div class="person">
                	<?PHP if($exeRow['personal_image'] !='' && $_SESSION['sess_is_user'] == 1) { ?>
			...
			<div class="more-profiles">
				...
				<?PHP while($exeIndRow = com_db_fetch_array($exeIndResult)){ ?>
				<div class="person">
                	<?PHP if($exeIndRow['personal_image'] !='' && $_SESSION['sess_is_user'] == 1) { ?>
					<div class="person-img">
